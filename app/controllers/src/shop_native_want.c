 ...
 * Read-only WANT projections: the open want list and one want's state.
 *
...
#define ZCL_HOTSWAP_PROBE_LEAF "app.shop.want.list"
#include "hotswap/hotswap.h"
static const struct zcl_hotswap_leaf_replacement k_shop_want_leaves[] = {
    { "app.shop.want.list", zcl_native_handle_shop_want_list },
    { "app.shop.want.status", zcl_native_handle_shop_want_status },
};
ZCL_HOTSWAP_EXPORT_LEAVES(k_shop_want_leaves,
                          sizeof(k_shop_want_leaves) / sizeof(k_shop_want_leaves[0]))
#endif /* ZCL_HOTSWAP_GEN */

#ifdef ZCL_HOTSWAP_MODULE_GEN
#include "hotswap/hotswap_module.h"
#include <stdio.h>
static const struct zcl_hotswap_leaf k_shop_want_module_leaves[] = {
    { "app.shop.want.list", zcl_native_handle_shop_want_list },
    { "app.shop.want.status", zcl_native_handle_shop_want_status },
};
/* Structural health hook: a table that lost a name or a body would
 * otherwise publish a leaf that dispatches into nothing. */
static bool shop_want_module_selftest(char *error, size_t error_cap)
{
    const size_t n = sizeof(k_shop_want_module_leaves) /
                     sizeof(k_shop_want_module_leaves[0]);
    for (size_t i = 0; i < n; i++) {
        if (!k_shop_want_module_leaves[i].name ||
            !k_shop_want_module_leaves[i].name[0] ||
            !k_shop_want_module_leaves[i].fn) {
            if (error && error_cap)
                (void)snprintf(error, error_cap,
                               "shop_want leaf %zu has no name or no body", i);
            return false;
        }
    }
    return true;
}
ZCL_HOTSWAP_MODULE_LEAVES(k_shop_want_module_leaves, shop_want_module_selftest)
