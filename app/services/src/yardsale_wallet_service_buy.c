    ...
    const uint8_t ad_root[32], int64_t now_unix, struct json_value *out)
{
    ...
    /* Claim the durable plan before key access or the outbound accept. Two
     * confirmations may validate concurrently, but only one can perform the
     * atomic PLANNED -> ARMING transition. ARMING remains sticky if a later
     * step fails because retrying an uncertain custody side effect is less
     * safe than requiring explicit operator inspection. */
    enum db_yardsale_plan_claim_result claim =
        db_yardsale_plan_claim(ndb, &p->row, now_unix);
    if (claim == DB_YARDSALE_PLAN_CLAIM_REFUSED)
        return yw_fail(out, YARDSALE_WALLET_ERR_PLAN_CONFLICT,
                       "another confirmation already owns this plan — "
                       "inspect its state before retrying");
    if (claim != DB_YARDSALE_PLAN_CLAIMED)
        return yw_fail(out, YARDSALE_WALLET_ERR_DB,
                       "the plan ledger could not claim this buy — no "
                       "accept was sent");
