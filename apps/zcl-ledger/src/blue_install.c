...
static int install(installer *device, const uint8_t *code,
    ...
    static const uint8_t flags[] = {0x04, 1, 0};
    params[params_length++] = 0x02;
    params[params_length++] = 5;
    memcpy(params + params_length, profile->version, 5);
    params_length += 5;
    memcpy(params + params_length, flags, sizeof flags);
    params_length += sizeof flags;
