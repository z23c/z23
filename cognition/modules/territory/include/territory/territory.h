 ...
 *
 *   engine/composition/lib_module_order.def   the SET of lib modules (via the code index's
 ...
 * engine/composition/lib_module_order.def and the app-shape list — no second list. */
 ...
 * while happening to mention core/modules/net in a baseline row would then be reported
 * as not binding core/modules/coins, which is a guess, and a wrong one. Only the first
