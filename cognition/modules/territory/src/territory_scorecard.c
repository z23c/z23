                   ...
                   int cap)
{
    ...
    for (int i = 0; i < ng && n < cap; i++) {
        /* The scorecard is a module report. Authority, context, and room rows
         * remain directly addressable navigation groups, but including them
         * here would mix hierarchy with the single-owner build modules this
         * roll-up compares. The module kind is generated from the one build
         * module declaration, so this is classification rather than a second
         * module list. */
        if (strcmp(groups[i].kind, "module") != 0)
            continue;
