...
static const uint8_t publication_receipt_magic[8] =
    {'Z','P','R','C','1',0,0,0};
const uint8_t publication_ack_set_magic[8] =
    {'Z','P','A','K','1',0,0,0};
...
/* A repo with no HEAD ref yet is about to take its first snapshot of the
 * whole worktree, which is the one call in the hot dev-loop path that is
 * not O(changed files); the caller logs the one-time cost. */
...
static void anchor_cycle_meta_build(const struct vcs_devloop_verdict *v,
