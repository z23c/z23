 ...
 * Purpose: ball — a native window on screen in under a second, from one
 * `make ball` typed after a clone.
 *
 * Two modes:
 *   ball                 open a real RGFW window and animate it until Esc,
 *                          close, or `--seconds=N` elapses. Prints one log
 *                          line the moment the first frame is presented.
 *   ball --frames=N      headless self-test: same painter, no window, no
 ...
 * loading on Linux, Win32 on Windows) in its software-surface mode: ball
...
#elif defined(BALL_UNDEF_MSC_VER)
#undef _MSC_VER
#undef BALL_UNDEF_MSC_VER
#endif

#define BALL_WIDTH 640u
#define BALL_HEIGHT 400u
#define BALL_TARGET_FPS 60

static uint8_t ball_pixels[BALL_WIDTH * BALL_HEIGHT *
			     BALL_PIXEL_BYTES];

static double ball_now_ms(void)
{
	struct timespec ts;
...
static void ball_wall_stamp(char *out, size_t cap)
{
	struct timespec ts;
	struct tm tm_utc;
...
static void ball_sleep_until(double deadline_ms)
{
	for (;;) {
		const double now = ball_now_ms();
		if (now >= deadline_ms)
			return;
		struct timespec ts;
...
static void ball_parse_options(int argc, char **argv,
				 struct ball_options *opts)
{
	...
	for (int i = 1; i < argc; i++) {
		...
		if (strncmp(arg, "--frames=", 9) == 0) {
			...
			(void)fprintf(stderr, "ball: unknown option %s\n",
				      arg);
			opts->show_help = 1;
			return;
		}
 ...
 */
static int ball_selftest(const struct ball_options *opts)
{
	struct ball_canvas canvas = {
		.pixels = ball_pixels,
		.width = BALL_WIDTH,
		.height = BALL_HEIGHT,
	};
	struct ball_world world;
	ball_world_init(&world, canvas.width, canvas.height);

	const unsigned frames = opts->frames ? opts->frames : 120u;
	const unsigned mid = frames > 1u ? frames / 2u : 0u;
	(void)printf("ball selftest: frames=%u size=%ux%u mode=headless\n",
		     frames, canvas.width, canvas.height);

	const double t0 = ball_now_ms();
	...
	for (unsigned i = 0; i < frames; i++) {
		ball_world_step(&world, canvas.width, canvas.height,
				  BALL_TEST_DT_SECONDS);
		ball_render(&world, &canvas,
			      (uint32_t)((double)i * 1000.0 / 60.0));

		const double start = ball_now_ms();
		/* "Present" = the frame left this process: fold it. */
		const uint64_t digest = ball_canvas_digest(&canvas);
		const double done = ball_now_ms();
		if (i == 0) {
			digest_first = digest;
		...
		for (size_t p = 3u; p < (size_t)canvas.width *
						...
						BALL_PIXEL_BYTES;
		     p += BALL_PIXEL_BYTES) {
			if (ball_pixels[p] != 0xFFu)
				alpha_ok = 0;
		}
		if (opts->print_every_frame) {
			(void)printf("ball present frame %u/%u in %.1f us\n",
				     i + 1u, frames, (done - start) * 1e3);
		}
	}
	const double total_ms = ball_now_ms() - t0;

	if (frames > 1u && digest_first == digest_mid) {
		(void)fprintf(stderr,
			      "ball selftest: FAIL frames did not move "
	...
	if (!alpha_ok) {
		(void)fprintf(stderr,
			      "ball selftest: FAIL canvas is not opaque\n");
		return 1;
	}

	(void)printf("ball selftest: OK frames=%u first_present_ms=%.2f "
...
static int ball_window(const struct ball_options *opts)
{
	struct ball_canvas canvas = {
		.pixels = ball_pixels,
		.width = BALL_WIDTH,
	...
	char stamp[40];
	ball_wall_stamp(stamp, sizeof(stamp));
	const double t0 = ball_now_ms();

	RGFW_window *win = RGFW_createWindow(
		"ball - Z23 prompt-to-pixel", (i32)0, (i32)0,
		(i32)canvas.width, (i32)canvas.height,
		(RGFW_windowFlags)RGFW_windowCenter);
	if (win == NULL) {
		(void)fprintf(stderr,
			      "ball: RGFW_createWindow failed — run "
			      "`ball --frames=N` for the headless proof\n");
		return 1;
	}
	const double window_ms = ball_now_ms() - t0;
	...
	RGFW_surface *surface = RGFW_window_createSurface(
		...
		RGFW_formatRGBA8);
	if (surface == NULL) {
		(void)fprintf(stderr, "ball: RGFW_window_createSurface "
				      "failed\n");
	...
	for (;;) {
		...
		if (RGFW_window_shouldClose(win))
			goto presented;
		if (deadline != 0.0 && ball_now_ms() >= deadline)
			goto presented;
		...
		const double present_start = ball_now_ms();
		RGFW_window_blitSurface(win, surface);
		const double present_done = ball_now_ms();
		...
		if (frames == 0) {
			...
			ball_wall_stamp(stamp, sizeof(stamp));
			(void)printf(
				"ball %s present frame 0 in %.1f ms "
		...
		next_frame_ms += 1000.0 / (double)BALL_TARGET_FPS;
		ball_sleep_until(next_frame_ms);
		last_ms = ball_now_ms();
...
presented:;
	const double total_ms = ball_now_ms() - t0;
	(void)printf("ball closed after %llu frames in %.1f ms "
...
static void ball_print_usage(void)
{
	(void)printf("ball — the prompt-to-pixel demo\n"
		     "usage: ball [--frames=N] [--seconds=S] [--quiet]\n"
...
{
	struct ball_options opts;
	ball_parse_options(argc, argv, &opts);
	if (opts.show_help) {
		ball_print_usage();
		return 2;
	}
	return opts.frames ? ball_selftest(&opts) : ball_window(&opts);
}
