...
source bitmap as PNG entirely in memory, writes the native image clipboard,
and leaves the window open. Linux also advertises 24-bit BMP as a compatibility
fallback; Windows publishes PNG and DIB. Success changes both the button pixels
and title immediately. The bounded encoders can report or fill the exact byte
count in caller-owned memory without touching a file or clipboard.
