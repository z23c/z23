 ...
 * The wallet's read-only SECURITY POSTURE: keystore presence and lock state. Every mutating sibling in this file is
...
ZCL_HOTSWAP_LEAVES_BEGIN(wallet_security)
ZCL_HOTSWAP_LEAF("core.wallet.security.status", zcl_native_handle_wallet_security_status)
ZCL_HOTSWAP_LEAVES_END(wallet_security)
#endif
