 ...
 * window and a monotonic seq. core/modules/net is ranked below contexts/commons/modules/vcs
 * (engine/composition/lib_module_order.def) and may not include it, so the
 ...
 * in net/onion_peer_merge.h and implemented in core/modules/net/src/onion_peer_merge.c.
 ...
 * writes znam_names and is NOT re-decided here — core/modules/net ranks below
 * contexts/naming/modules/znam, and a second answer to "is this a legal name" is a second
 ...
 * addrman_publish_reputation_weights (core/modules/net/src/addrman.c), which is
 ...
 * path (see the seam note in engine/composition/src/boot_endpoint_records.c). */
