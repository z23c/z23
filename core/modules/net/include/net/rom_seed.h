 ...
 * service (core/modules/net/file_service.c) drives the serve path through the decision
 ...
 * datadir root): where engine/composition/src/boot_bundle_fetch.c lands verified swarm
 ...
 *
 * A ".zvsb" file is the compressed transport contexts/commons/modules/vcs/src/source_bundle.c
 ...
 *
 * core/modules/net may not include contexts/commons/modules/vcs (the two modules are deliberately
 * independent — see engine/composition/lib_module_order.def), so the four numbers below
 ...
 * rehashes the entire manifest and every blob. tests/harness/src/
...
struct rom_artifact {
     ...
     * not an authority: nothing in core/modules/net accepts bytes because of it. */
 ...
 * rom_seed_register. The rotation path (engine/composition/src/bundle_exporter_generations.c bx_rotate)
