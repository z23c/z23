 ...
 *
 * Inline twin of the site design system's tokens (contexts/explorer/views/src/site.css).
 * These pages live in core/modules/net — below the app/views layer — and the front
 ...
 * (core/modules/net/src/site_routes.c) walks the nav table expanded from
...
static size_t serve_directory_json(uint8_t *response, size_t max)
{
    ...
    while (AR_STEP_ROW_READONLY(s) == SQLITE_ROW &&
         ...
         * connman seed-fetch string-scan relies on (core/modules/net/src/connman.c
                                    ...
                                    uint8_t *response, size_t response_max)
{
    return onion_render_response((char *)response,response_max,
        ...
        "<script type='engine/application/json' id='pow-challenge'>"
 ...
 * single registry. core/modules/net must not include app/ headers (layering), so
