...
selftest() { local u; selftest_tmp="$(mktemp -d "${TMPDIR:-/tmp}/z23-provision.XXXXXX")"; trap '[ -z "${selftest_tmp:-}" ] || rm -rf -- "$selftest_tmp"' EXIT; valid_size 6G && ! valid_size 6T && [ "$(size_bytes 6G)" -eq 6442450944 ] || die "size selftest"; u="$(mount_unit_for /mnt/z23-dev-operator)"; render_mount_unit /mnt/z23-dev-operator 6G 1000 1000 >"$selftest_tmp/$u"; grep -q 'size=6G,mode=0700,uid=1000,gid=1000' "$selftest_tmp/$u" || die "mount selftest"; ! grep -Eq '\.zclassic|systemctl (restart|stop).*zclassic' "$selftest_tmp/$u" || die "authority selftest"; say "selftest PASS"; }
