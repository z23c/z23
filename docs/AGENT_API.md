...
`engine/composition/commands/*.def` and do not appear in `z23 discover help` — so
...
The native RPC contracts are implemented in `cognition/controllers/src/agent_controller.c`
for the agent map/build surface, `cognition/controllers/src/agent_contracts_controller.c`
for the versioned contract registry, and `cognition/controllers/src/agent_ops_controller.c`
for the focused no-jq command center. `cognition/controllers/src/agent_diagnose_controller.c`
...
`cognition/controllers/include/controllers/agent_contracts.def`; runtime availability,
...
`cognition/controllers/src/agent_contract_registry.c` with direct native fields
...
command rows into `engine/entry/main.c` usage text.
...
`cognition/controllers/include/controllers/agent_operator_contracts.h`, shared by
...
The static no-cookie native commands in `engine/entry/main.c` use one
...
`engine/controllers/src/api_controller_app_protocols.c`; the same rows feed
...
`engine/controllers/src/api_controller_service_catalog.c` and is exposed as
...
belong in C under `cognition/controllers/src/agent_controller.c`,
`cognition/controllers/src/agent_contracts_controller.c`, and
`cognition/controllers/src/agent_interface_controller.c`; compact
operator/architecture answers that should not require `jq` belong in
`cognition/controllers/src/agent_ops_controller.c`, then get exposed through native
commands and REST. Registry-backed command groupings for `agentmap` and
telemetry live in `cognition/controllers/src/agent_contract_registry.c`
(`g_agent_command_surfaces`); `agentinterface` capability rows are emitted
from `cognition/controllers/src/agent_contract_capability_registry.c`, while
...
- Focused test routing is DRY: both native `z23 agentimpact` and
  ...
  `cognition/controllers/include/controllers/agent_impact_rules.def`. Add a rule
...
`platform/deploy/examples/zclassic23-self-update.timer` as
...
`platform/deploy/examples/zclassic23-remote-test-node.service` as
