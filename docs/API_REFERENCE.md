...
| `zcode work publish` | ready | mutate / app-write / **owner** · foreground/moderate | `workspace`, `work`, `datadir`, **`job_root`**, `details`, `path`, `kind`, `namespace`, `transport_root`, `include_evidence_wires`, `board`, `action_id` | `zcl.zcode_work_publish.v1` | `z23 zcode work publish --input='<typed continuation from work accept>'` | Continue publication of one exact accepted work |
