...
| `dev land` | compat 🔧 → `z23-dev dev land` | mutate / dev-mutation / operator · persistent/high | **`action`**, `tip`, `worktree`, `note`, `seq`, `json` | `zcl.land.v1` | `z23-dev dev land drive` | Queue a tip; attach signed Git intent, prove and push without waiting — *the landing queue is a development-lane coordination surface* |
...
| `ops mesh roster` (aliases: `fleet.roster`) | ready | read / read / operator · fast/low | `datadir`, `after` | `zcl.fleet_roster.v1` | `z23 fleet roster [--after=<paired rows already listed>]` | List this operator's machines with each fact labelled by who established it |
