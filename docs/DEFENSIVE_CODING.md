...
| **`check-windows-acceptance`** | FAIL | Cross-compiles the platform-seam acceptance catalog (`lib/platform/tests/windows_acceptance.mk`, 51 programs in `lib/test/src/`, `lib/platform/tests/` and `lib/base/tests/`) for Windows with mingw. Exists because the catalog itself was invoked by **nothing** -- `windows-acceptance-compile` appeared in a `.PHONY` list and in no gate, no CI path and no script, so 51 acceptance programs with real per-test source lists stood in for verification of the macOS/Windows seam without a compiler reading them on a clean tree. A catalog nothing runs is the same false green as no catalog. Compiles at the product's own API floor, `$(ZCL_WINDOWS_API_FLOOR)`, shared with `ZCL_PLATFORM_CPPFLAGS` so the two cannot drift: they already had, because `217d3eb6e` raised the product to the Windows 10 baseline and left the catalog compiling against `0x0600`, grading a Windows the product does not ship. Compile, not execute: these are Windows binaries and this is a POSIX host, so the honest deliverable is a cross-link and it is labelled as one; `make windows-acceptance` additionally runs them under Wine and REFUSES rather than skipping when Wine is absent. **UNOBSERVED contract:** with no mingw toolchain it prints `UNOBSERVED`, in that word, and exits 0 -- an outside contributor is never blocked by a cross-compiler they do not have -- but UNOBSERVED is not a pass and is not cached. | `lib/platform/tests/windows_acceptance.mk` |
