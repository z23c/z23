...
| **`check-winacceptance`** | FAIL | Builds all 26 platform-seam acceptance programs in `tools/winacceptance/` (24) and `tools/tests/` (2). Exists because until it did, **nothing built or ran any of them** -- no target, no test group, no CI reference, and outside its own directory the string `winacceptance` appeared nowhere in the tree; 26 programs no compiler had ever seen were standing in for the verification of the macOS/Windows seam. Wiring them found real defects in 11 of the 26, including `logical_cpu_acceptance.c` calling `GetActiveProcessorCount`, an API mingw declares only at `_WIN32_WINNT >= 0x0601` while the product is pinned at `0x0600` -- an assertion that could never run at the real target. Cross-compiles all 26 with mingw at the build's own `ZCL_PLATFORM_CPPFLAGS` API floor (parsed from the Makefile, never copied here), natively compiles the portable ones, and EXECUTES only the six a POSIX host can honestly run. It reports compiled / executed / compile-checked-only as three separate numbers and never sums them: a Windows binary does not run here, so for 20 of the 26 the honest deliverable is a compile check and it is labelled as one. A program whose manifest bucket says it runs but which exits 77 is graded a FAILURE, not a skip. The manifest is reconciled against the directories, so a new program with no row is a `rc=2` FATAL rather than another silently-unbuilt file -- that reconciliation is the part that stops this rotting a second time. **UNOBSERVED contract:** with no mingw toolchain it prints `UNOBSERVED`, in that word, and exits 0 -- an outside contributor is never blocked by a tool they do not have -- but UNOBSERVED is not a pass and is not cached. | `tools/scripts/winacceptance.sh` |
