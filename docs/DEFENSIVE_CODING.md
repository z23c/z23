...
| **`check-macos-acceptance`** | HARD | The **static** half of `tools/scripts/macos_acceptance.sh`, run for real. `config/platform/macos_capabilities.def` is the closed declarative darwin-arm64 truth: 13 capability rows each carry a legal state, typed reason, and registered evidence, while eight exact `ZCL_MACOS_REQUIRED_TEST(...)` rows pin the documented baseline groups not already selected by capability evidence. Their sorted union is exactly 37 macOS evidence groups; deletion, unknown-group substitution, and same-size registered exact-set substitution all fail. An unavailable capability still names refusal evidence—`self_backtrace` therefore proves the macOS stub boundary, not Linux signal-context behavior. **UNOBSERVED contract:** the native leg—building and executing all 37 exact groups with zero skips on darwin-arm64—is reported `UNOBSERVED`, in that word, on Linux; `make macos-acceptance` closes it on an arm64 Mac. The static self-test drives 11 positive/adversarial fixtures through the shipped validator. | `tools/lint/check_macos_acceptance.sh` |
