...
| **`check-capability-inventory-generated`** | HARD | `docs/CAPABILITY_INVENTORY.jsonl` is GENERATED evidence, never authored findings. The C23 generator rescans maintained public headers and packages, exposed symbols, direct call/include use sites, canonical registered-test reachability, normalized function bodies, and contract-like untested declarations; then the gate diffs every byte. Header prose, name-based reachability, and alpha-shape matches carry explicit `UNPROVEN` ceilings. It fails loud below census/test-root floors or when any registered root is missing. Fix with `make docs-capability-inventory`, never by editing the JSONL. Hermetic `--selftest` proves a planted row fails. No baseline or override. |
