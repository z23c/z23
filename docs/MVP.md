...
| 5 | **List + sell file via store** | Operator lists product → buyer pays shielded → buyer receives file — tests: `tests/harness/src/test_store_e2e_gate.c` via **CI slice: `make ci-mvp-gates`**. The legacy `store_e2e` selector proves in-process store persistence, memo-bound reconcile, token credit, dedupe, and token-gated binary file bytes. The `store_e2e_shielded` selector adds the real shielded-payment teeth: a Sapling output is encrypted to a merchant wallet, `wallet_try_sapling_decrypt` recovers value+memo params-free, the recovered note is persisted, and `db_store_received_payment_for_memo` credits only the matching `ZCL23ORDER:<order_id>` memo while the legacy address+amount finder over-counts a same-address wrong-order payment. Still ◐: this is hermetic/in-process, not yet a full live buyer over the store/onion/file-transfer path. **2026-09-10, lane `storelive` audit — the remaining gap is a named owner gate, not lane work.** A buyer node cannot reach a seller node's store at all today, and the blocker is structural in three independent places: the HTTPS listener answers `405 Method Not Allowed` to every non-GET/HEAD request before any routing happens (`core/modules/net/src/https_server.c:826`); the `STORE` flavor is absent from that listener's dispatch table entirely — `ZCL_HTTPS_DISPATCH_STORE` expands to a comment (`core/modules/net/src/https_server.c:924-926`) and every site handler it *does* dispatch is called with a `NULL, 0` body (`:933`, `:947`, `:961`), so no request body could reach the handler even if the row were admitted; and the only transport that reaches `store_handle_request` is the in-process onion dynhost callback (`core/modules/net/src/onion_service.c:1197-1202` — there is no `bind`/`accept` for it anywhere), whose outbound client is GET-only with no method or body argument (`core/modules/net/src/tor_fetch.c:21-34` → `dynhost_client_fetch(addr, 80, path, cb, ctx, timeout)`), while order creation requires POST (`engine/controllers/src/store_controller.c:202-208`). Two further facts the store never exposes over the wire: `product.token_id` (rendered only as prose, `contexts/explorer/views/src/store_view.c:512`) and `product.content_hash` (rendered only on the too-large-to-inline page, `contexts/explorer/views/src/store_view.c:707-720`, never for a product small enough to actually deliver), so a remote buyer must be told the expected SHA3-256 out of band rather than discover it. **C5 is therefore ◐ BLOCKED(owner: core unseal)** — the decision is the owner's: either unseal `core/modules/net` for a request-bearing loopback/HTTPS `STORE` dispatch (fail-closed — loopback peer plus regtest or an explicit operator flag that is refused by name on mainnet, with the refusal pinned by a test), or extend the `vendor/tor` dynhost client with a method + body so a buyer can POST over the seller's onion. Until one of those lands, the partial two-node witness that IS reachable — an independent buyer node with its own wallet paying a seller's order memo on a shared regtest chain, the seller reconciling it, and a wrong-memo payment left uncredited — is `make test-store-two-node-acceptance`, which prints `VERDICT=PARTIAL reason=no_remote_store_transport` and can never print PASS. **A third blocker found the same day while building that partial witness, and it invalidates the rung-A proof this cell has been resting on.** The merchant leaf `app.store.list-product` opens `node.db` in the CLI process (`sn_open_db` at `engine/controllers/src/store_native_handlers.c:445` → `node_db_open_runtime`), and `node.db` is under a deliberate single-owner lease: a second process is refused with `DATABASE_OWNERSHIP_CONFLICT: canonical database owner already holds path=<datadir>/node.db` (`engine/models/src/database_owner_lease.c:318-324`), which the handler reports as `STORE_NOT_INITIALISED` (`store_native_handlers.c:137-139`). So **no product can be listed through the shipped CLI while the node is running** — reproduced live 2026-09-10 on a fresh isolated regtest datadir, unconditional, not a lock that clears with retry (60 s of bounded retry, every attempt refused). The buyer leaves are unaffected because they proxy the `storebuy_*` RPCs and run inside the node (`contexts/market/controllers/src/store_buyer_controller.c:475-479`); it is the merchant half that is stranded. The consequence for this cell: `tools/scripts/store_sell_operator_proof.sh` calls `app store list-product` against its own booted node at line 377, so `make test-store-operator-proof` — the "rung A" full-binary operator proof named at the top of this cell — cannot currently pass either. C5's slice gates (`store_e2e`, `store_e2e_shielded`) are hermetic and in-process, so they are untouched and remain the regression floor; but until the merchant leaf can reach a live node's store, C5 has no runnable full-binary proof at all, single-node or two-node. Fixing the merchant leaf (route it through an RPC the way the buyer leaves already are) is `engine/controllers` work and does NOT need the core unseal; the remote-buyer transport above still does. | ◐ |
...
via the genuine wipe-to-tip stopwatch (336 s < 600 s budget against the
dedicated fixture peer, 2026-09-09; ledger `verdict=pass`). Remaining
◐ criteria: **C5** (hermetic real ivk-decrypt + memo-bound store slice exists;
