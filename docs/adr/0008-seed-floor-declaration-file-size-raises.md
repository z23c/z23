...
- **Related:** [`0006-always-sync-spine-file-size-raises.md`](./0006-always-sync-spine-file-size-raises.md),
  ...
  `engine/services/src/bg_validation_service.c` (the walk-extent consumer).
...
ceiling for `app/`+`engine/composition/src/`) ratchets: the enforced-tier baseline "can
...
- **`engine/services/src/block_index_loader_rebuild.c` 840 → 853 (+13)** —
...
- **`engine/composition/src/consensus_state_snapshot_install_activate.c` 1158 → 1169
