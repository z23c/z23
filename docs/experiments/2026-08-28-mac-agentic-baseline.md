...
- *(this slice)* — suppress `sync_violation_lag` outbound peer rotation while a staged checkpoint bundle is waiting for headers to reach the checkpoint, so the ZCL23 fast-sync peer feeding those headers is not disconnected during the long header crawl. Also add `ZCL_SERVICE_ADDNODE_PEERS` for multiple snapshot-seed peers, and stop `tools/dev/grok_report.c` from being linked into the node binary.
