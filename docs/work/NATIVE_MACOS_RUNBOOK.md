...
| `check-file-size-ceiling` drift after adding macOS branches | Inline `#if defined(__APPLE__)` blocks grew existing units | Put the next variant behind a per-platform seam file or header seam. Raising a record again requires an ADR; shrinking is always preferred |
