...
| ChaCha20-Poly1305 — `lib/crypto/src/chacha20poly1305.c` | SSE2 four-block word-sliced tier, explicitly selectable | mandatory x86-64 ABI plus a one-time portable-oracle KAT | NEON four-block word-sliced tier, explicitly selectable; **AUTO remains portable pending physical promotion evidence** | mandatory arm64 ABI plus the same one-time KAT; a divergent bit disables the vector tier | `test_chacha20_isa_parity`: RFC vector, every length 0..1024, randomized differential cases, in-place operation, AEAD caller boundaries, and counter-exhaustion refusal | the oracle reports a 65,520-byte throughput sample; caller-shaped p95 and worst-case measurements remain the promotion gate |
...
- **secp256k1.** Upstream's field/scalar code over `__int128` limbs is already
  ...
  tier on both arches.
- **AES-256 / ChaCha20-Poly1305 / Curve25519 / Ed25519 / SHA-512 / SHA-1 /
  RIPEMD-160 / BLAKE2s / BLAKE3.** No SIMD tier exists on **either** arch.
