...
| SHA-512 — `core/modules/crypto/src/sha512.c` | portable | n/a | **FEAT_SHA512** (`vsha512hq`/`vsha512h2q` and `vsha512su0q`/`vsha512su1q`) | either macOS SHA-512 sysctl plus a one-time compression KAT against portable C; any mismatch stays portable | `test_sha512_isa_parity`: independent reachability, published KATs, every length 0..1024, randomized/chunked parity, and teeth | `make bench-simd` — the SHA-512 row; measured on the arm64 Mac at about 1.48 GB/s vs 0.79 GB/s portable for 1 KiB messages (1.88x) |
...
| SHA-1 / RIPEMD-160 — `core/modules/crypto/src/sha1.c`, `ripemd160.c` | portable — no tier on either arch | n/a | portable | n/a | the KAT vectors inside `test_crypto` | none |
...
- **SHA-512 FEAT_SHA512** — `core/modules/crypto/src/sha512.c`. Re-derive:
  `git grep -n "vsha512hq\|vsha512su" -- core/modules/crypto/src/sha512.c`.
