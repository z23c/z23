...
fixes. It is not a READY=1 contract miss for that peer's bisect.

Isolated pair-probe binds only 39250+ quads. Published `P2P_PORT` values for
an operator's other mesh members are never bind candidates (one real
deployment observed distinct published ports across four peers, none in the
39250+ probe range). Dialing a peer's onion is a client path and does not
bind that peer's published port locally.
