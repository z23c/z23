...
/* Grant the worker role, through the role seam (util/fleet_role_check.h),
...
/* Grandfather walks that stopped before the end of what they were
 * reading — the key set filled, the batch bound ran out, or a stored
 * row would not decode. Above zero means some peer key was never
 * looked at and its rows are being refused for that reason.
 * Reported by `fleet ledger status` as `grandfather_truncated`. */
