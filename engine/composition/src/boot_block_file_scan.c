 ...
 * ZClassic block headers out of the raw blk*.dat files (in parallel) and
 * drives the passes that mark BLOCK_HAVE_DATA, resolve orphan pprev links
 * from disk, and propagate cumulative chain metadata so
 * find_most_work_chain can pick the best tip. It owns the block-file magic
 * and header-read-size macros; the parsed-metadata structs and the apply
 * half are boot_block_index_apply.c. None are shared with the
 * reindex/rebuild core in boot_index.c.
 *
 ...
 * scan_apply_one_file — live in engine/composition/src/boot_block_index_apply.c
 ...
 * block is in the map — live in engine/composition/src/boot_block_index_ancestry.c
