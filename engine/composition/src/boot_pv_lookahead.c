 ...
 * pre-verification pool (engine/jobs/pv_lookahead.c). The pool itself was already
 ...
 * (engine/composition/src/boot_mint_anchor.c), so on an ordinary replay the consume hook in
 * proof_validate's step (engine/jobs/src/proof_validate_stage.c) ran on every
 ...
 * default_verify_tx (engine/jobs/src/proof_validate_verify.c) as Groth16 PUBLIC
