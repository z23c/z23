 ...
 *
 * Purpose: the exporter's private SQLite VFS shim — the sqlite3_vfs vtable
 * whose xOpen hands every database I/O to the retained anonymous staging
 * descriptor, with every non-file method forwarded to the platform base VFS,
 * plus the per-binding registration that installs it under a unique name.
 *
 * Split out of consensus_state_snapshot_export.c along the file-size ceiling
 * seam (E1). That file keeps the output binding lifecycle (name validation,
 * directory bind, staging create/seal/publish) and the export entry point;
 * this TU is the whole sqlite3_vfs surface and nothing else. Only
 * output_vfs_register crosses the seam, declared in
