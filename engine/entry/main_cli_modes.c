 ...
 *
 * Offline CLI modes split from engine/entry/main.c; each exits without booting the
 ...
 * core/modules/coins/src/utxo_commitment.c:utxo_commitment_sha3_compute()
 * so the computed SHA3 equals the compile-time checkpoint at
 * core/modules/chain/src/checkpoints.c (when run at the anchor height).
 */
struct gen_snap_ctx {
    FILE *out;
...
int import_complete_shielded_mode(int argc, char **argv)
{
     ...
     * persisted index entry (core/modules/validation/src/accept_block_header.c) and
...
int mintutxocommitment_mode(int argc, char **argv)
{
    ...
    printf("block_hash=%s\nsha3=%s\n\n", bh_hex, s3_hex);
    printf("/* Paste into core/modules/chain/src/checkpoints.c as the latest\n"
