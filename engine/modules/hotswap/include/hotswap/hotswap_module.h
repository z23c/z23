 ...
 *     (engine/composition/hotswap_eligible.def) is dispatched against the registry's public
 ...
 *     dispatch drains (epoch/refcount quiesce in engine/modules/kernel/command_registry.c).
 *
...
/* Forward decls only — engine/modules/hotswap never pulls kernel/app headers. The concrete
 ...
 * ROOT folds, but in cognition/modules/codeindex's source-Merkle dialect (tagged leaf/node
...
struct hotswap_activate_report {
     ...
     * core/modules/crypto nor its dispatch table. Either field is "" on any path that
 ...
 * test harness so engine/modules/hotswap stays free of kernel headers. Returns true on
 ...
 * callback: engine/modules/hotswap must not link the kernel registry. */
 ...
 * engine/composition/hotswap_swappable.def whose TU never emits `zcl_hotswap_module` at
 ...
 * `leaf` is on the compiled swappable allowlist (engine/composition/hotswap_swappable.def)
...
/* The repo-relative source TU that owns `leaf` per engine/composition/hotswap_swappable.def,
 ...
 * engine/composition/hotswap_islands.def and are compiled into the same -Bsymbolic .so. */
...
/* The probe leaf engine/composition/hotswap_eligible.def declares for `source_tu`, or NULL
