...
/* ── swappable allowlist, compiled from engine/composition/hotswap_swappable.def ───────
...
/* ── probe map, compiled from engine/composition/hotswap_eligible.def ─────────────────
...
#include "../../../engine/composition/hotswap_probe_cases.def"
 ...
 *     files, folded in cognition/modules/codeindex's source-Merkle dialect instead of the
    ...
    if (!probe || !probe[0]) {
        ...
        if (why && why_cap)
            snprintf(why, why_cap,
                     ...
                     "engine/composition/hotswap_eligible.def", module->source_tu);
        return false;
    }
...
/* Longest canonical leaf path in engine/composition/hotswap_swappable.def is well under
 ...
 *
 * A leaf may appear under exactly ONE source_tu (engine/composition/hotswap_swappable.def
 ...
 * (tests/harness/src/test_hotswap_module_v2.c).
 *
                              ...
                              struct hotswap_activate_report *report)
{
     ...
     * sources plus --gc-sections, and pulling in core/modules/crypto's SHA-256 (with
