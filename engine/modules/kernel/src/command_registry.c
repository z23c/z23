 ...
 * That number is not protecting a parser (platform/modules/json bounds nesting depth, not
                                         ...
                                         char *why, size_t why_size)
{
    ...
    for (size_t i = 0; i < input->num_children; i++) {
        ...
        if (strcmp(key, "files") == 0) {
             ...
             * shop_want_v1, contexts/market/models/include/models/shop_want.h). Without
             ...
             * (engine/controllers/src/swap_controller.c), which refuses anything
             * outside 1..1000000. */
            type_ok = value->type == JSON_INT && json_get_int(value) >= 1 &&
                      json_get_int(value) <= 1000000;
        } else if (strcmp(key, "price_per_mb_zat") == 0) {
            /* app.market.offer asking price per MB in ZATOSHIS (int64 in
             * struct file_offer, core/modules/net/include/net/file_market.h). Without
             * this rule the default branch demands a string and rejects the
             ...
             * low-rank command kernel depend upward on contexts/explorer/modules/presentation. */
         ...
         * default STRING branch makes the leaf uninvokable from a shell. */
            ...
            /* The telemetry cursor form is a non-negative INT the leaf
             * issued (`since=0` is "I have nothing"); the carve-out exists
             * because the default branch demands STRING, which made the
             * CLI's `--since=41` uninvokable. code.recent reuses the key
             * for a bounded commit-ish STRING. Handlers own semantics. */
            type_ok = value->type == JSON_INT
