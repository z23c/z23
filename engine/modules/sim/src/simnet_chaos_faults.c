 ...
 * originals are file-static) from four already-reviewed regression tests,
 * so the row shapes and column lists are proven-correct, not reinvented:
 *   - tests/harness/src/test_block_index_loader.c (genesis-root seed_tip fixture)
 *   - tests/harness/src/test_segment_corruption.c (seal/tamper/detect/repair)
 *   - tests/harness/src/test_supervisor.c         (freeze/deadline-stall pattern)
 *   - tests/harness/src/test_condition_engine.c   (condition + EV_OPERATOR_NEEDED
 *                                              counting pattern)
 */
 ...
 * (tools/scripts/check_lib_layering.sh) — engine/modules/sim must not depend on the
 ...
 * Signature verified against engine/services/include/services/
 ...
 * tests/harness/src/test_helpers.c symbols, only linked into the test_zcl /
 ...
 * declared under ZCL_TESTING (platform/modules/util/include/util/supervisor.h); this
 * file also compiles into the production zclassic23 binary (engine/modules/sim is a
...
bool chaos_fault_stall_single_stage(struct chaos_fault_result *out)
{
     ...
     * exact static instance (matches tests/harness/src/test_condition_engine.c's
...
bool chaos_fault_kill_restart_mid_recovery(struct chaos_fault_result *out)
{
     ...
     * (engine/jobs/src/stage_rederive_range.c) rewinds the body-dependent
