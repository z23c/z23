 ...
 * (engine/reducer/jobs/src/reducer_frontier.c), so a transparent set that is wrong BELOW
 ...
 * engine/reducer/jobs/include/jobs/reducer_frontier.h reducer_frontier_derive_coins_best
 ...
 * (cursor_in 0 -> next_cursor 1). See engine/controllers/src/sovereignty_controller.c
 ...
 * epilogue (engine/services/src/reindex_epilogue.c) calls this right after a full
...
/* ── ANCHOR-SET MINT writer (engine/modules/storage/src/coins_kv_snapshot_write.c) ──
 *
 * Stream the LIVE coins_kv set to a UTXO snapshot sidecar in the exact
 * on-disk format the loader (core/modules/chain/src/utxo_snapshot_loader.c) reads and
