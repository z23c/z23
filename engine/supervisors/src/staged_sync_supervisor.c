 ...
 *   - "stage_spin_<name>" (engine/reducer/services/src/reducer_drain.c): fires when a
 ...
 *   - "reducer_drive_stuck" (engine/reducer/conditions/src/reducer_drive_watchdog.c):
 ...
 * (platform/modules/util/src/supervisor.c). Computed here from staged_stage_tick every
...
static void staged_stage_tick(struct liveness_contract *c)
{
     ...
     * 2s tick and full validation is never altered. platform/modules/util/src/
