...
static const unsigned char *at(const struct img *im, uint64_t off, uint64_t len)
{
    if (len > im->n)
        return NULL;
    if (off > im->n - len)
...
/* Fixed-width little-endian reads come from lib/base, not from a private
...
static bool dynstr_copy(const struct img *im, uint64_t stroff, uint64_t strsz,
...
static bool dynstr_equals(const struct img *im, uint64_t stroff, uint64_t strsz,
...
static bool dynsym_count_from_hash(const struct img *im, uint64_t hash_off,
                                   uint64_t *out_count)
{
...
static bool dynsym_count_from_gnu_hash(const struct img *im, uint64_t gh_off,
                                       uint64_t *out_count)
{
