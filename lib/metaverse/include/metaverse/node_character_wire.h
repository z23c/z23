 ...
 * clock. lib/astro builds its trigonometric tables on first use, though, so a
 ...
 * lib/astro computes. House cusps are absent because equal houses make every
 ...
 * lib/astro's accepted range, so a chart cast for it is never refused for its
 ...
 * birthday (astro_instant_is_valid), or a chart lib/astro refuses. It never
