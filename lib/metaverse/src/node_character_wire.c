 ...
 * file in lib/astro and lib/metaverse. */
 ...
 * each folded into lib/astro's accepted range so a chart cast for the result is
 ...
 * astro_position's fields are bounded by lib/astro's own contract, so the
 * narrowing cannot lose information; the caller re-checks the range anyway,
 * because a lib/astro defect should be refused here rather than encoded. */
