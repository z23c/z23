 ...
 *
 * Paid-file delivery, BUYING side: ask another node for one chunk.
 *
 * Split from file_market_delivery.c along the wire, not along a size
 * boundary. Everything here runs as the party that WANTS bytes, so it
 * holds no handler table and asks no moderation question: what a remote
 * node is willing to serve is that node's own decision, arriving as a
 * reply status this code reports verbatim rather than reinterprets.
 *
 * Deadlines are the other reason this side is its own file. Every call
 * takes an absolute monotonic deadline and must distinguish "the peer
 * said no" from "we ran out of time", because collapsing the two would
 * let a slow link read as a refusal — and a node on a slow disk or a
 * long circuit is a first-class participant here. */
