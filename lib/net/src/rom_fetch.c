...
static void rf_conn_drop(struct rf_peer_conn *c)
{
    if (!c || !c->open)
        return;
...
static bool rf_conn_ensure(struct rf_peer_conn *c, const char *addr,
                           uint16_t port)
{
...
static enum rf_xchg rf_chunk_exchange(struct rf_peer_conn *c,
