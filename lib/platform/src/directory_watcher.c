                                     ...
                                     const char *root)
{
    ...
    struct watcher_state *s = wide ? calloc(1, sizeof(*s)) : NULL;
...
static bool add_tree(struct watcher_state *s, const char *path)
{
    ...
    struct watch_item *items = realloc(s->items, (s->count + 1) * sizeof(*items));
    if (!items) { inotify_rm_watch(s->fd, wd); return false; }
    s->items = items; s->items[s->count].wd = wd;
    s->items[s->count].path = strdup(path);
                                     ...
                                     const char *root)
{
    ...
    struct watcher_state *s = calloc(1, sizeof(*s));
    if (!s) return false;
        ...
        if (rc > 0) {
            ...
            for (size_t at = 0; at + sizeof(struct inotify_event) <= (size_t)n;) {
                ...
                if ((e->mask & (IN_CREATE | IN_MOVED_TO)) && (e->mask & IN_ISDIR) && e->len) {
                    ...
                    if (base) { size_t z=strlen(base)+strlen(e->name)+2; char *child=malloc(z);
