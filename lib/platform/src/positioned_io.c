                                 ...
                                 uint64_t offset)
{
    ...
    intptr_t native = _get_osfhandle(fd);
    if (native == -1) return -1;
    HANDLE positioned = ReOpenFile((HANDLE)native, GENERIC_READ,
        FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
        FILE_FLAG_OVERLAPPED);
    if (positioned == INVALID_HANDLE_VALUE) return -1;
    OVERLAPPED operation = {0};
    ...
    if (!operation.hEvent) { CloseHandle(positioned); return -1; }
    DWORD read = 0;
    BOOL ok = ReadFile(positioned, data, (DWORD)size, &read, &operation);
    if (!ok && GetLastError() == ERROR_IO_PENDING)
        ok = GetOverlappedResult(positioned, &operation, &read, TRUE);
    CloseHandle(operation.hEvent);
    CloseHandle(positioned);
