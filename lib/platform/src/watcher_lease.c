 ...
 * Purpose: Bind watcher launch and stop operations to private capabilities. */
                          ...
                          uint64_t *high)
{
    ...
    wchar_t *wide = count > 0
        ? zcl_malloc((size_t)count * sizeof(*wide), "watcher-root-path") : NULL;
...
static bool parent_matches(const struct watcher_record *record)
{
    ...
    bool ok = process && QueryFullProcessImageNameA(process, 0, image, &count) &&
              ...
              GetLastError() == ERROR_INSUFFICIENT_BUFFER;
    if (ok) {
        parent = zcl_malloc(parent_size, "watcher-parent-token");
        own = zcl_malloc(own_size, "watcher-own-token");
    }
    ok = ok && parent && own &&
...
#endif
    struct watcher_record *saved = zcl_malloc(sizeof(*saved),
                                               "watcher-launch-record");
                                   ...
                                   const char *image, const char hash[65])
{
    ...
    if (ok) {
        struct platform_watcher_accepted_binding *accepted =
            zcl_calloc(1, sizeof(*accepted), "watcher-accepted-binding");
