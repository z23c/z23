 ...
 * `app/models/include/models/activerecord.h`.
 *
 * Coverage:
...
{
    if (!ch_write(HAVE_FIX, "app/models/include/models/arfix.h", ARFIX_H))
        return false;
    ...
    for (int i = 0; i < n; i++) {
        ...
        snprintf(rel, sizeof(rel), "app/models/src/%s.c", callers[i]);
    ...
    for (int i = 0; i < CH_MENTIONS; i++) {
        ...
        snprintf(rel, sizeof(rel), "app/models/src/%s.c", mentions[i]);
        if (!ch_write(HAVE_FIX, rel, body)) return false;
    }
    return ch_write(HAVE_FIX, "lib/net/src/foo.c", NET_FOO_C) &&
           ch_write(HAVE_FIX, "lib/net/src/purpose_long.c", PURPOSE_LONG_C) &&
           ch_write(HAVE_FIX, "lib/net/src/purpose_huge.c", PURPOSE_HUGE_C);
}
...
int test_code_have(void)
{
        ...
        if (ci) {
            ...
            if (at >= 0) {
                ...
                CH_CHECK("validates_presence_of belongs to that header",
                         vfound &&
                         (strcmp(vs.def_path,
                                 "app/models/include/models/activerecord.h") == 0 ||
                          strcmp(vs.decl_path,
                                 "app/models/include/models/activerecord.h") == 0));
    ...
    /* 7: purpose capacity, measured against real stored text. */
    {
        struct ci_file f;
        bool found = false;
        bool ok = codeindex_file(fx, "lib/net/src/purpose_long.c", &f, &found);
        ...
        found = false;
        ok = codeindex_file(fx, "lib/net/src/purpose_huge.c", &f, &found);
    ...
    int at = n > 0 ? ch_find(caps, n, "app/models/include/models/arfix.h") : -1;
    ...
    if (at >= 0) {
        ...
        CH_CHECK("an example caller is named",
                 ...
                 strstr(caps[at].example_caller, "app/models/src/") != NULL);
    ...
    /* 6a: the warm (cached) answer equals the cold one. */
    {
        ...
        int wat = wn > 0 ? ch_find(wcaps, wn,
                                   "app/models/include/models/arfix.h") : -1;
