...
int test_devagent_surface(void)
{
    ...
    TEST("checkout root: found by walking up from a nested directory") {
        ...
        ASSERT(tds_touch(dir, "config/commands/root.def"));
        ASSERT(tds_touch(dir, "tools/dev/test_group_catalog.def"));
        (void)snprintf(nested, sizeof(nested), "%s/config/commands", dir);
    ...
    TEST("checkout root: a partial marker set is walked past, not accepted") {
        ...
        ASSERT(tds_touch(outer, "config/commands/root.def"));
        ...
        ASSERT(tds_touch(inner, "config/commands/root.def"));
        (void)snprintf(deep, sizeof(deep), "%s/inner/config/commands", outer);
