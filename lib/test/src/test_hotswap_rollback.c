 ...
 * module .so images built by the Makefile from lib/test/fixtures/
 ...
 * the quiescence hook. Those are caller-provided by design (lib/hotswap never
 ...
 * probe leaf in config/hotswap_eligible.def is core.status, and both leaves
 * used below are on that row — the same identity the fixture module stamps
 * itself with. */
#define RB_TU        "app/controllers/src/status_native_handlers.c"
