 ...
 * Purpose: ball's frame painter. No windowing API appears in this file, so
...
static const struct ball_rgb ball_stop_a = {0x12, 0x14, 0x28};
static const struct ball_rgb ball_stop_b = {0x2f, 0x6f, 0x5a};

static double ball_wrap01(double v)
{
	double frac = v - (double)(int64_t)v;
	return frac < 0.0 ? frac + 1.0 : frac;
}

static uint8_t ball_channel(uint8_t a, uint8_t b, double t)
{
	return (uint8_t)((double)a + ((double)b - (double)a) * t + 0.5);
}

static uint8_t ball_clamp_channel(int v)
{
	if (v < 0)
		return 0;
	if (v > 255)
...
{
	const double side = (double)BALL_SQUARE_SIDE;
		   ...
		   const struct ball_canvas *canvas, uint32_t t_ms)
{
	...
	const double phase = ball_wrap01((double)t_ms / 8000.0);
	uint8_t top_r = ball_channel(ball_stop_a.r, ball_stop_b.r, phase);
	uint8_t top_g = ball_channel(ball_stop_a.g, ball_stop_b.g, phase);
	uint8_t top_b = ball_channel(ball_stop_a.b, ball_stop_b.b, phase);
	uint8_t bot_r = ball_channel(ball_stop_b.r, ball_stop_a.r, phase);
	uint8_t bot_g = ball_channel(ball_stop_b.g, ball_stop_a.g, phase);
	uint8_t bot_b = ball_channel(ball_stop_b.b, ball_stop_a.b, phase);

	for (uint32_t y = 0; y < h; y++) {
		const double t = (h > 1u) ? (double)y / (double)(h - 1u) : 0.0;
		const uint8_t r =
			ball_channel(top_r, bot_r, t);
		const uint8_t g =
			ball_channel(top_g, bot_g, t);
		const uint8_t b =
			ball_channel(top_b, bot_b, t);
		uint8_t *row = pixels + (size_t)y * (size_t)w *
					 BALL_PIXEL_BYTES;
		for (uint32_t x = 0; x < w; x++) {
			uint8_t *px = row + (size_t)x * BALL_PIXEL_BYTES;
			px[0] = r;
			px[1] = g;
			px[2] = b;
			px[3] = 0xFF;
	 ...
	 * clipped by hand because ball has no scissor rect. */
	const int32_t side = (int32_t)BALL_SQUARE_SIDE;
	const int32_t x0 = (int32_t)(world->x + 0.5);
	const int32_t y0 = (int32_t)(world->y + 0.5);
	const uint8_t pulse = ball_clamp_channel(
		110 + (int)(70.0 * ball_wrap01((double)t_ms / 1200.0)));
