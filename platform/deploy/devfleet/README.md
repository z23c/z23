...
Run the node under systemd, never from a shell. `platform/deploy/zcl23-devfleet@.service`
is a template unit for exactly this:

```bash
install -m644 platform/deploy/zcl23-devfleet@.service ~/.config/systemd/user/
...
```bash
install -D -m644 platform/deploy/zclassic23-spinning-disk-watchdog.conf \
...
`WATCHDOG=1` heartbeat in `engine/composition/src/boot_sd_watchdog.c`, sent every
