...
#
#   platform/packaging/install/install.sh   the ~30-line shim served at the domain. It
...
# z23_front_door test group (tests/harness/src/test_z23_front_door.c), which can
...
case_cut_shim_installs() {
    ...
    bash "$REPO_ROOT/platform/packaging/release/build_release.sh" --front-door \
...
case_boot_unreachable() {
    ...
    grep -qxE 'z23-pin-v1:[0-9a-f]{64}:[0-9a-f]{64}' "$SCRIPT_DIR/RELEASE_PIN" \
        || die "platform/packaging/install/RELEASE_PIN is not a z23-pin-v1 record"
