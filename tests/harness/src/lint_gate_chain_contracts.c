 ...
 * engine/services/src so the gate's `find app -name '*.c'` scan sees it. */
#define RRUNG_FIXTURE_DST "engine/services/src/_repair_rung_fixture_tmp_reconcile.c"
#define BORROWED_SEED_SCRIPT_REL "tools/lint/check_no_new_borrowed_seed.sh"
/* A production-scope caller planted under engine/services/src so the borrowed-seed
 * ratchet's app/config/lib/tools scan sees it. */
#define BORROWED_SEED_FIXTURE_DST \
    "engine/services/src/_borrowed_seed_fixture_tmp.c"
#define COIN_BACKFILL_CALLER_SCRIPT_REL \
    "tools/lint/check_no_new_coin_backfill_caller.sh"
/* Production-scope callers planted under engine/services/src and domain/ so the
...
#define COIN_BACKFILL_CALLER_FIXTURE_DST \
    "engine/services/src/_coin_backfill_caller_fixture_tmp.c"
#define COIN_BACKFILL_CALLER_DOMAIN_FIXTURE_DST \
    "contexts/wallet/domain/src/_coin_backfill_caller_fixture_tmp.c"
...
int t_no_new_coin_backfill_caller(void)
{
    ...
    char root[PATH_MAX], engine_dir[PATH_MAX], jobs_dir[PATH_MAX];
    char src_dir[PATH_MAX], reducer_dir[PATH_MAX], reducer_jobs_dir[PATH_MAX];
    char reducer_src_dir[PATH_MAX], def_path[PATH_MAX], allowed_path[PATH_MAX];
    int root_resolved = repo_path(root, sizeof(root), COIN_BACKFILL_ROOT_REL);
    int engine_n = root_resolved == 0
        ? snprintf(engine_dir, sizeof(engine_dir), "%s/engine", root) : -1;
    int jobs_n = engine_n > 0 && engine_n < (int)sizeof(engine_dir)
        ? snprintf(jobs_dir, sizeof(jobs_dir), "%s/engine/jobs", root) : -1;
    int src_n = jobs_n > 0 && jobs_n < (int)sizeof(jobs_dir)
        ? snprintf(src_dir, sizeof(src_dir), "%s/engine/jobs/src", root) : -1;
    int reducer_n = src_n > 0 && src_n < (int)sizeof(src_dir)
        ? snprintf(reducer_dir, sizeof(reducer_dir), "%s/engine/reducer", root)
        : -1;
    int reducer_jobs_n = reducer_n > 0 && reducer_n < (int)sizeof(reducer_dir)
        ? snprintf(reducer_jobs_dir, sizeof(reducer_jobs_dir),
                   "%s/engine/reducer/jobs", root) : -1;
    int reducer_src_n = reducer_jobs_n > 0 &&
                        reducer_jobs_n < (int)sizeof(reducer_jobs_dir)
        ? snprintf(reducer_src_dir, sizeof(reducer_src_dir),
                   "%s/engine/reducer/jobs/src", root) : -1;
    int def_n = src_n > 0 && src_n < (int)sizeof(src_dir)
        ? snprintf(def_path, sizeof(def_path),
                   "%s/stage_repair_coin_backfill.c", src_dir) : -1;
    int allowed_n = reducer_src_n > 0 &&
                    reducer_src_n < (int)sizeof(reducer_src_dir) &&
                    def_n > 0 && def_n < (int)sizeof(def_path)
        ? snprintf(allowed_path, sizeof(allowed_path),
                   "%s/stage_repair_reducer_frontier_coin.c",
                   reducer_src_dir) : -1;
    int paths_ok = allowed_n > 0 && allowed_n < (int)sizeof(allowed_path);
    int dirs_ok = paths_ok &&
        (mkdir(root, 0700) == 0 || errno == EEXIST) &&
        (mkdir(engine_dir, 0700) == 0 || errno == EEXIST) &&
        (mkdir(jobs_dir, 0700) == 0 || errno == EEXIST) &&
        (mkdir(src_dir, 0700) == 0 || errno == EEXIST) &&
        (mkdir(reducer_dir, 0700) == 0 || errno == EEXIST) &&
        (mkdir(reducer_jobs_dir, 0700) == 0 || errno == EEXIST) &&
        (mkdir(reducer_src_dir, 0700) == 0 || errno == EEXIST);
    const char *def_body =
    ...
    if (paths_ok) {
        ...
        (void)rmdir(reducer_src_dir);
        (void)rmdir(reducer_jobs_dir);
        (void)rmdir(reducer_dir);
        (void)rmdir(src_dir);
        (void)rmdir(jobs_dir);
        (void)rmdir(engine_dir);
        (void)rmdir(root);
    }
...
int t_chain_evidence_reconstruct_uses_retry_persistence(void)
{
    ...
    TEST("chain evidence reconstruct uses retry persistence") {
        ...
        ASSERT(repo_path(path, sizeof(path),
                         "engine/services/src/chain_evidence_reconstruct.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
...
int t_boot_genesis_init_preserves_restored_authority_contract(void)
{
    ...
    TEST("boot genesis init preserves restored non-genesis authority tip") {
        ...
        buf = NULL;
        ASSERT(repo_path(path, sizeof(path),
                         "engine/services/src/utxo_recovery_restore.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
...
int t_sha3_window_tool_check_contract(void)
{
    ...
    TEST("gen_sha3_windows supports single-window source proof checks") {
        ...
        ASSERT(read_entire_file(path, &buf) == 0);
        ASSERT(strstr(buf, "core/modules/chain/src/sha3_windows.c") != NULL);
        PASS();
    } _test_next:;
    free(buf);
    return failures;
...
int t_block_index_flat_atomic_save_contract(void)
{
    ...
    TEST("block index flat save is a single atomic embedded-header file") {
        char path[PATH_MAX];
        ASSERT(repo_path(path, sizeof(path),
                         "engine/services/src/block_index_flat_save.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
...
int t_process_block_split_uses_reducer_language(void)
{
    ...
    TEST("process-block split comments name reducer, not deleted engines") {
        const char *files[] = {
            ...
            "core/modules/validation/include/validation/process_block_internals.h",
            ...
            "core/modules/validation/src/process_block_tip_child.c",
            "core/modules/validation/src/process_block_tip_publish.c",
        };
        const char *stale[] = {
            "connect_tip",
            "disconnect_tip",
...
int t_projection_deferral_is_not_block_rejected_contract(void)
{
    ...
    TEST("projection deferral is chain advance diagnostic, not block reject") {
        ...
        buf = NULL;
        ASSERT(repo_path(path, sizeof(path),
                         "engine/controllers/src/sync_controller_blocks.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ASSERT(strstr(buf, "block_source_policy_note_projection_deferred") != NULL);
        ...
        buf = NULL;
        ASSERT(repo_path(path, sizeof(path),
                         "engine/services/src/block_source_policy_runtime.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
...
int t_gap_fill_wakes_connman_dispatch_contract(void)
{
    ...
    TEST("gap-fill requeue wakes connman dispatch") {
        char path[PATH_MAX];
        ASSERT(repo_path(path, sizeof(path),
                         "engine/services/src/gap_fill_service.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ...
        ASSERT(repo_path(path, sizeof(path),
                         "core/modules/net/src/connman.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ASSERT(strstr(buf, "connman_wake_message_handler") != NULL);
...
int t_msg_process_yields_to_send_phase_contract(void)
{
    ...
    TEST("msg_process yields to send phase under recv backlog") {
        ...
        ASSERT(repo_path(path, sizeof(path),
                         "core/modules/net/include/net/msgprocessor.h") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
