...
static bool tx_type_registry_row_fields_ok(
    const struct zcl_command_registry *registry,
    const struct zcl_transaction_type_contract *types, size_t i)
{
    return types[i].id && types[i].id[0] &&
...
static bool tx_type_registry_id_unique(
    const struct zcl_transaction_type_contract *types, size_t i)
{
    bool ok = true;
...
static bool tx_type_registry_leaf_commands_exist(
    const struct zcl_command_registry *registry)
{
    return zcl_command_registry_find(
...
static int check_api_tx_type_registry_refs_real_leaves(void)
{
    int failures = 0;
    ...
    printf("api: transaction type registry references real native leaves... ");
    {
        ...
        for (size_t i = 0; ok && i < count; i++)
            ok = tx_type_registry_row_fields_ok(registry, types, i) &&
                 tx_type_registry_id_unique(types, i);
        ok = ok && tx_type_registry_leaf_commands_exist(registry);
        if (ok) printf("OK\n");
        else { printf("FAIL\n"); failures++; }
    }
    return failures;
...
static bool tx_type_micro_lab_profile_row_ok(
    const struct zcl_transaction_micro_lab_profile *profile,
    int expected_slot)
{
    const struct zcl_transaction_type_contract *type =
        zcl_transaction_type_find(profile->type_id);
    return profile->first_slot == expected_slot &&
           profile->last_slot >= profile->first_slot &&
           profile->recipient_zat ==
               ZCL_TRANSACTION_MICRO_LAB_RECIPIENT_ZAT &&
           profile->fee_zat == ZCL_TRANSACTION_MICRO_LAB_FEE_ZAT &&
           type && strcmp(type->availability, "ready") == 0;
}

static bool tx_type_micro_lab_profiles_cover_slots(
    ...
    size_t profile_count)
{
    ...
    for (size_t i = 0; ok && i < profile_count; i++) {
        ok = tx_type_micro_lab_profile_row_ok(&profiles[i], expected_slot);
        expected_slot = profiles[i].last_slot + 1;
    }
    return ok && expected_slot == ZCL_TRANSACTION_MICRO_LAB_TARGET + 1 &&
...
static bool tx_type_micro_lab_slot91_shape_ok(const struct json_value *data,
    const struct json_value *profile, const struct json_value *type,
    const struct json_value *guide_input)
{
    return data && profile && type && guide_input &&
           ...
           json_get_int(json_get(data,
               "campaign_transaction_type_count")) == 14 &&
...
static bool tx_type_micro_lab_slot91_payload_ok(
    const struct json_value *data, const struct json_value *profile,
    const struct json_value *type, const struct json_value *guide_input,
    const char *output)
{
    return strcmp(json_get_str(json_get(profile, "transaction_type")),
                  "htlc_redeem") == 0 &&
...
static bool tx_type_micro_lab_slot91_ok(
    const struct zcl_command_registry *registry,
    const struct zcl_command_spec *spec,
    struct zcl_command_context *context)
{
    ...
    char why[160];
    bool ok = spec && zcl_command_registry_input_validate(
    ...
    ok = ok && tx_type_micro_lab_slot91_shape_ok(
            data, profile, type, guide_input) &&
         tx_type_micro_lab_slot91_payload_ok(
            data, profile, type, guide_input, output);
    json_free(&root);
    return ok;
}

static bool tx_type_micro_lab_slot101_fail_closed(
    const struct zcl_command_registry *registry,
    const struct zcl_command_spec *spec,
    struct zcl_command_context *context)
{
...
static int check_api_tx_type_micro_lab_covers_100_slots(void)
{
    int failures = 0;
    ...
    printf("api: native micro-lab covers 100 exact slots and fails closed... ");
    {
        ...
        ok = ok && tx_type_micro_lab_profiles_cover_slots(
            profiles, profile_count);
        ok = ok && tx_type_micro_lab_slot91_ok(registry, spec, &context);
        ok = ok && tx_type_micro_lab_slot101_fail_closed(
...
static bool tx_type_alias_row_ok(
    const struct zcl_command_registry *registry,
    const struct zcl_transaction_command_alias *aliases, size_t i)
{
    return zcl_transaction_type_find(aliases[i].type_id) != NULL &&
           zcl_command_registry_find(registry,
...
static bool tx_type_alias_row_unique(
    const struct zcl_transaction_command_alias *aliases, size_t i)
{
    bool ok = true;
    for (size_t j = 0; ok && j < i; j++)
...
static bool tx_type_command_roles_match(
    const struct zcl_transaction_type_contract *type)
{
    bool ok = true;
    if (type->builder_command[0])
        ok = (zcl_transaction_type_command_roles(
            type, type->builder_command) &
            ZCL_TRANSACTION_COMMAND_ROLE_BUILDER) != 0;
    if (ok && type->commit_command[0])
        ok = (zcl_transaction_type_command_roles(
            type, type->commit_command) &
            ZCL_TRANSACTION_COMMAND_ROLE_COMMIT) != 0;
    if (ok && type->inspect_command[0])
        ok = (zcl_transaction_type_command_roles(
            type, type->inspect_command) &
            ZCL_TRANSACTION_COMMAND_ROLE_INSPECT) != 0;
    return ok;
}

static bool tx_type_nonchain_row_ok(
    const struct zcl_command_registry *registry,
    const struct zcl_transaction_type_contract *types, size_t type_count,
    const struct zcl_transaction_nonchain_command *nonchain, size_t i)
{
    const struct zcl_command_spec *spec =
        zcl_command_registry_find(registry,
            nonchain[i].command_path, NULL);
    return spec && nonchain[i].category && nonchain[i].category[0] &&
           nonchain[i].explanation && nonchain[i].explanation[0] &&
           !transaction_command_is_mapped(types, type_count,
                                          nonchain[i].command_path);
}

static bool tx_type_nonchain_path_unique(
    const struct zcl_transaction_nonchain_command *nonchain, size_t i)
{
    bool ok = true;
    for (size_t j = 0; ok && j < i; j++)
...
static int check_api_tx_type_reverse_aliases_real_types(void)
{
    int failures = 0;
    ...
    printf("api: reverse transaction aliases reference real types and leaves... ");
    {
        ...
        for (size_t i = 0; ok && i < alias_count; i++)
            ok = tx_type_alias_row_ok(registry, aliases, i) &&
                 tx_type_alias_row_unique(aliases, i);
        size_t type_count = 0;
        const struct zcl_transaction_type_contract *types =
            zcl_transaction_type_catalog(&type_count);
        for (size_t i = 0; ok && i < type_count; i++)
            ok = tx_type_command_roles_match(&types[i]);
        ...
        for (size_t i = 0; ok && i < nonchain_count; i++)
            ok = tx_type_nonchain_row_ok(registry, types, type_count,
                                         nonchain, i) &&
                 tx_type_nonchain_path_unique(nonchain, i);
        if (ok) printf("OK\n");
        else { printf("FAIL\n"); failures++; }
    }
    return failures;
...
static bool tx_type_chain_command_is_candidate(
    const struct zcl_command_spec *spec)
{
    return spec->mode != ZCL_COMMAND_MODE_BRANCH &&
...
static size_t tx_type_count_uncovered_chain_commands(
    const struct zcl_command_registry *registry,
    const struct zcl_transaction_type_contract *types, size_t type_count)
{
    size_t uncovered = 0;
    for (size_t i = 0; i < registry->count; i++) {
        ...
        if (tx_type_chain_command_is_candidate(spec) &&
            !transaction_command_is_mapped(
...
static int check_api_tx_type_chain_cmds_have_reverse_map(void)
{
    int failures = 0;
    ...
    printf("api: every chain-shaped mutating command has a reverse mapping... ");
    {
        ...
        size_t uncovered = 0;
        if (ok)
            uncovered = tx_type_count_uncovered_chain_commands(
                registry, types, type_count);
        ok = ok && uncovered == 0;
        if (ok) printf("OK\n");
        else { printf("\nFAIL (%zu uncovered)\n", uncovered); failures++; }
    }
...
static bool tx_type_wire_catalog_header_ok(const struct json_value *root)
{
    return strcmp(json_get_str(json_get(root, "schema")),
...
static bool tx_type_wire_legacy_v1_ok(const struct json_value *legacy_v1)
{
    return legacy_v1 &&
...
static bool tx_type_wire_legacy_v2_ok(const struct json_value *legacy_v2)
{
    return legacy_v2 &&
...
static bool tx_type_wire_overwinter_ok(const struct json_value *overwinter)
{
    return overwinter &&
...
static bool tx_type_wire_sapling_ok(const struct json_value *sapling)
{
    return sapling &&
...
static bool tx_type_wire_families_ok(const struct json_value *root)
{
    ...
    return tx_type_wire_legacy_v1_ok(legacy_v1) &&
           tx_type_wire_legacy_v2_ok(legacy_v2) &&
           tx_type_wire_overwinter_ok(overwinter) &&
           tx_type_wire_sapling_ok(sapling);
}

static bool tx_type_wire_script_nonstandard_ok(
    const struct json_value *nonstandard)
{
    return nonstandard &&
...
static bool tx_type_wire_script_nulldata_ok(const struct json_value *nulldata)
{
    return nulldata &&
...
static bool tx_type_wire_script_multisig_ok(const struct json_value *multisig)
{
    return multisig &&
...
static bool tx_type_wire_scripts_ok(const struct json_value *root)
{
    ...
    return scripts && json_size(scripts) == (size_t)TX_NULL_DATA + 1 &&
           tx_type_wire_script_nonstandard_ok(nonstandard) &&
           tx_type_wire_script_nulldata_ok(nulldata) &&
           tx_type_wire_script_multisig_ok(multisig);
}

static bool tx_type_wire_codecs_ok(const struct json_value *root)
{
...
static int check_api_tx_type_wire_catalog_eras_carriers(void)
{
    int failures = 0;
    ...
    printf("api: wire catalog covers source-defined eras and open-ended carriers... ");
    {
        ...
        bool ok = zcl_transaction_wire_catalog_json(&root) &&
            tx_type_wire_catalog_header_ok(&root) &&
            tx_type_wire_families_ok(&root) &&
            tx_type_wire_scripts_ok(&root) &&
            tx_type_wire_codecs_ok(&root);
        json_free(&root);
        if (ok) printf("OK\n");
        else { printf("FAIL\n"); failures++; }
    }
...
static bool tx_type_index_shape_ok(const struct json_value *root,
                                  const struct json_value *types)
{
    int64_t count = json_get_int(json_get(root,
                                          "transaction_type_count"));
    return types && types->type == JSON_ARR &&
           count == (int64_t)json_size(types) && count >= 30 &&
           count == json_get_int(json_get(root, "ready_count")) +
                      json_get_int(json_get(root,
                                            "process_only_count")) +
                      json_get_int(json_get(root,
                                            "contained_count")) +
...
static bool tx_type_index_proof_counts_ok(const struct json_value *root)
{
    return json_get_int(json_get(root, "transaction_type_count")) == 39 &&
...
static bool tx_type_index_policy_ok(const struct json_value *root)
{
    return strcmp(json_get_str(json_get(root, "wire_catalog_command")),
...
static bool tx_type_index_core_rows_ok(const struct json_value *types)
{
    ...
    bool ok = transparent && zanc_digest &&
...
static bool tx_type_index_app_rows_ok(const struct json_value *types)
{
...
static bool tx_type_index_zdir_zid_rows_ok(const struct json_value *types)
{
    ...
    bool ok = zdir_register && zdir_deregister &&
...
static bool tx_type_index_blog_zpay_rows_ok(const struct json_value *types)
{
...
static bool tx_type_body_hides_secrets(const char *body)
{
    return strstr(body, "private_key") == NULL &&
           strstr(body, "grant_token") == NULL &&
           strstr(body, "/home/") == NULL;
}

static int check_api_tx_type_collection_readiness_proof(void)
{
    int failures = 0;
    ...
    printf("api: transaction type collection separates readiness and proof... ");
    {
        ...
        ok = ok && tx_type_index_shape_ok(&root, types) &&
             tx_type_index_proof_counts_ok(&root) &&
             tx_type_index_policy_ok(&root);
        ok = ok && tx_type_index_core_rows_ok(types) &&
             tx_type_index_app_rows_ok(types) &&
             tx_type_index_zdir_zid_rows_ok(types) &&
             tx_type_index_blog_zpay_rows_ok(types);
        ok = ok && tx_type_body_hides_secrets(body);
        json_free(&root);
        if (ok) printf("OK\n");
        else { printf("FAIL\n"); failures++; }
    }
...
static int check_api_tx_type_list_fits_response_budget(void)
{
    int failures = 0;
...
static int check_api_tx_type_wire_fits_response_budget(void)
{
    int failures = 0;
...
static bool tx_type_guide_exec_ok(
    const struct zcl_command_registry *registry,
    const struct zcl_command_spec *spec,
    struct zcl_command_context *context,
    char *output, size_t output_cap, struct json_value *root)
{
    struct json_value input;
    json_init(&input);
    json_set_object(&input);
    ...
    size_t n = spec ? zcl_command_registry_execute_json(
        registry, spec, context, &input, false, spec->path, "normal",
        0, 0, NULL, output, output_cap - 1, &exit_code) : 0;
    json_free(&input);
    output[n < output_cap ? n : output_cap - 1] = 0;
    json_init(root);
    return n > 0 && n <= ZCL_COMMAND_EXTENDED_LIST_BUDGET &&
        n <= (size_t)spec->budget_bytes &&
        exit_code == ZCL_COMMAND_EXIT_OK && json_read(root, output, n) &&
        json_get_bool(json_get(root, "ok"));
}

static bool tx_type_guide_roles_present(
    const struct json_value *data, const struct json_value *type,
    const struct json_value *contracts, const struct json_value *builder,
    const struct json_value *submit, const struct json_value *inspect)
{
    return data && type && contracts && builder && submit && inspect &&
           strcmp(json_get_str(json_get(data, "schema")),
                  ZCL_TRANSACTION_TYPE_GUIDE_SCHEMA) == 0 &&
           strcmp(json_get_str(json_get(type, "id")),
                  "sapling_z_to_t") == 0 &&
...
static bool tx_type_guide_submit_path_ok(
    const struct json_value *data, const struct json_value *builder,
    const struct json_value *submit)
{
    return strcmp(json_get_str(json_get(builder, "command")),
                  "vault.intent.plan") == 0 &&
...
static bool tx_type_guide_inspect_and_schema_ok(
    const struct json_value *data, const struct json_value *builder,
    const struct json_value *inspect, const char *output)
{
    return strstr(json_get_str(json_get(inspect, "semantics")),
...
static int check_api_tx_type_guide_joins_live_contracts(void)
{
    int failures = 0;
    ...
    printf("api: one-call transaction guide joins exact live command contracts... ");
    {
        ...
        char output[ZCL_COMMAND_EXTENDED_LIST_BUDGET + 1];
        struct json_value root;
        bool ok = tx_type_guide_exec_ok(
            registry, spec, &context, output, sizeof(output), &root);
        const struct json_value *data = json_get(&root, "data");
        const struct json_value *type = data ?
            json_get(data, "transaction_type") : NULL;
        ...
        ok = ok && tx_type_guide_roles_present(
                data, type, contracts, builder, submit, inspect) &&
             tx_type_guide_submit_path_ok(data, builder, submit) &&
             tx_type_guide_inspect_and_schema_ok(
                data, builder, inspect, output);
        json_free(&root);
        if (ok) printf("OK\n");
        else { printf("FAIL\n"); failures++; }
    }
...
static bool tx_type_guide_fits_budget_for_id(
    const struct zcl_command_registry *registry,
    const struct zcl_command_spec *spec,
    struct zcl_command_context *context, const char *type_id)
{
    struct json_value input;
    struct json_value root;
    char output[ZCL_COMMAND_EXTENDED_LIST_BUDGET + 1];
    ...
    json_init(&input);
    json_set_object(&input);
    json_push_kv_str(&input, "type", type_id);
    ...
    output[n < sizeof(output) ? n : sizeof(output) - 1] = 0;
    json_init(&root);
    bool ok = n > 0 && n <= (size_t)spec->budget_bytes &&
         exit_code == ZCL_COMMAND_EXIT_OK &&
         json_read(&root, output, n) &&
         json_get_bool(json_get(&root, "ok"));
    const struct json_value *data = json_get(&root, "data");
    const struct json_value *type = data ?
        json_get(data, "transaction_type") : NULL;
    ok = ok && type &&
         strcmp(json_get_str(json_get(type, "id")), type_id) == 0;
    json_free(&root);
    return ok;
}

static int check_api_tx_type_every_guide_fits_budget(void)
{
    int failures = 0;
    ...
    printf("api: every transaction guide fits its declared response budget... ");
    {
        ...
        bool ok = spec && types && type_count == 39 &&
                  spec->budget_bytes == ZCL_COMMAND_EXTENDED_LIST_BUDGET;
        for (size_t i = 0; ok && i < type_count; i++)
            ok = tx_type_guide_fits_budget_for_id(
                registry, spec, &context, types[i].id);
        if (ok) printf("OK\n");
        else { printf("FAIL\n"); failures++; }
    }
    return failures;
...
static bool tx_type_command_mapped_send_shape_ok(
    const struct json_value *data, const struct json_value *mappings,
    const struct json_value *transparent)
{
    return data && mappings && transparent &&
           strcmp(json_get_str(json_get(data, "schema")),
...
static bool tx_type_command_mapped_send_roles_ok(
    const struct json_value *transparent)
{
    return api_test_array_has_str(json_get(transparent, "roles"),
                                  "component") &&
...
static bool tx_type_command_mapped_send_ok(
    const struct zcl_command_registry *registry,
    const struct zcl_command_spec *spec,
    struct zcl_command_context *context)
{
    ...
    const struct json_value *transparent = mappings ?
        ...
        : NULL;
    ok = ok && tx_type_command_mapped_send_shape_ok(
            data, mappings, transparent) &&
         tx_type_command_mapped_send_roles_ok(transparent);
    json_free(&root);
    return ok;
}

static bool tx_type_command_vault_send_route_ok(
    const struct zcl_command_registry *registry,
    const struct zcl_command_spec *spec,
    struct zcl_command_context *context)
{
    ...
    output[n < sizeof(output) ? n : sizeof(output) - 1] = 0;
    json_init(&root);
    bool ok = n > 0 && exit_code == ZCL_COMMAND_EXIT_OK &&
         json_read(&root, output, n);
    const struct json_value *data = json_get(&root, "data");
    const struct json_value *mappings = data ?
        json_get(data, "transaction_types") : NULL;
...
static bool tx_type_command_address_new_nonchain_ok(
    const struct zcl_command_registry *registry,
    const struct zcl_command_spec *spec,
    struct zcl_command_context *context)
{
...
static bool tx_type_command_qr_show_nonchain_ok(
    const struct zcl_command_registry *registry,
    const struct zcl_command_spec *spec,
    struct zcl_command_context *context)
{
...
static int check_api_tx_type_reverse_lookup_fail_closed(void)
{
    int failures = 0;
    ...
    printf("api: reverse command lookup is mapped, routed, or fail-closed unclassified... ");
    {
        ...
        bool mapped_ok = tx_type_command_mapped_send_ok(
            registry, spec, &context);
        bool route_ok = tx_type_command_vault_send_route_ok(
            registry, spec, &context);
        bool addr_ok = tx_type_command_address_new_nonchain_ok(
            registry, spec, &context);
        bool qr_ok = tx_type_command_qr_show_nonchain_ok(
            registry, spec, &context);
        bool ok = mapped_ok && route_ok && addr_ok && qr_ok;
        if (ok) printf("OK\n");
        else { printf("FAIL\n"); failures++; }
    }
    return failures;
...
static bool tx_type_member_zcode_ok(uint8_t *response, size_t response_size)
{
    size_t n = api_handle_request(
        "GET", "/api/v1/transaction-types/zcode_release_anchor",
        NULL, 0, response, response_size);
    const char *body = api_test_body(response, n, response_size);
    struct json_value root;
    json_init(&root);
    bool ok = n > 0 && body && json_read(&root, body, strlen(body));
    ok = ok && strcmp(json_get_str(json_get(&root, "schema")),
...
static bool tx_type_member_blog_ok(uint8_t *response, size_t response_size)
{
    size_t n = api_handle_request(
...
static bool tx_type_member_market_ok(uint8_t *response, size_t response_size)
{
    size_t n = api_handle_request(
...
static bool tx_type_member_htlc_ok(uint8_t *response, size_t response_size)
{
    size_t n = api_handle_request(
...
static bool tx_type_member_unknown_404(uint8_t *response, size_t response_size)
{
    size_t n = api_handle_request("GET",
                               "/api/v1/transaction-types/not_real",
                               NULL, 0, response, response_size);
    response[n < response_size ? n : response_size - 1] = '\0';
    return strstr((char *)response,
                  "HTTP/1.1 404 Not Found") != NULL;
}

static int check_api_tx_type_member_exact_unknown_404(void)
{
    int failures = 0;
    ...
    {
        static uint8_t response[262144];
        bool zcode_ok = tx_type_member_zcode_ok(response, sizeof(response));
        bool blog_ok = tx_type_member_blog_ok(response, sizeof(response));
        bool market_ok = tx_type_member_market_ok(
            response, sizeof(response));
        bool htlc_ok = tx_type_member_htlc_ok(response, sizeof(response));
        bool missing_ok = tx_type_member_unknown_404(
            response, sizeof(response));
        bool ok = zcode_ok && blog_ok && market_ok && htlc_ok && missing_ok;
        if (ok) printf("OK\n");
        else { printf("FAIL\n"); failures++; }
    }
    return failures;
...
{
    int failures = 0;
    failures += check_api_tx_type_registry_refs_real_leaves();
    failures += check_api_tx_type_micro_lab_covers_100_slots();
    failures += check_api_tx_type_reverse_aliases_real_types();
    failures += check_api_tx_type_chain_cmds_have_reverse_map();
    failures += check_api_tx_type_wire_catalog_eras_carriers();
    failures += check_api_tx_type_collection_readiness_proof();
    failures += check_api_tx_type_list_fits_response_budget();
    failures += check_api_tx_type_wire_fits_response_budget();
    failures += check_api_tx_type_guide_joins_live_contracts();
    failures += check_api_tx_type_every_guide_fits_budget();
    failures += check_api_tx_type_reverse_lookup_fail_closed();
    failures += check_api_tx_type_member_exact_unknown_404();
    return failures;
}
