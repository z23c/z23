...
static bool write_cap_budget_fixture(void)
{
    ...
    n += (size_t)snprintf(hot + n, sizeof(hot) - n,
                          "/* core/modules/net/src/cap_hot.c — budget-shrink fixture. */\n"
    ...
    for (int i = 0; i < 10; i++)
        n += (size_t)snprintf(hot + n, sizeof(hot) - n,
            ...
            i);
    n += (size_t)snprintf(hot + n, sizeof(hot) - n, "}\n");
    ok = ok && cap_budget_mk_write(CAP_BUDGET_FIX, "core/modules/net/src/cap_hot.c", hot);
    ...
    ok = ok && cap_budget_mk_write(CAP_BUDGET_FIX,
                                   "core/modules/net/include/net/cap_hot.h", hoth);

    /* 10 long-named padding headers, all included by cap_hot.c via depfile. */
    char depfile[4096];
    n = 0;
    n += (size_t)snprintf(depfile + n, sizeof(depfile) - n,
        "build/obj/cap_hot.o: core/modules/net/src/cap_hot.c "
        "core/modules/net/include/net/cap_hot.h");
    for (int i = 0; i < 10; i++) {
        char hdr_rel[256];
        snprintf(hdr_rel, sizeof(hdr_rel),
            "core/modules/net/include/net/padding_header_number_%02d_with_a_very_long_"
    ...
    for (int i = 0; i < 10; i++) {
        ...
        snprintf(rel, sizeof(rel),
            "core/modules/net/src/caller_module_for_budget_shrink_fixture_number_%02d_"
            "with_padding.c", i);
        char c[1024];
        snprintf(c, sizeof(c),
            "#include \"net/cap_hot.h\"\n"
...
static int test_code_capsule_static_id_is_exact(void)
{
    ...
    TEST("code_capsule: stable id selects one same-named static definition "
        ...
        const char *id =
            "fn:static:engine/modules/storage/src/wallet_projection.c:ensure_schema";
        struct json_value input;
        json_init(&input); json_set_object(&input);
...
static int test_code_change_plan_patch(void)
{
    ...
    TEST("code_change_plan: unified patch extracts exact bounded paths") {
        ...
        (void)json_push_kv_str(&input, "patch",
            "diff --git a/platform/modules/json/src/json.c b/platform/modules/json/src/json.c\n"
            "--- a/platform/modules/json/src/json.c\n+++ b/platform/modules/json/src/json.c\n@@ -1 +1 @@\n");
        struct zcl_command_request request = {
            .input = &input, .view = "normal",
            .invoked_name = "code.change-plan",
        };
