...
/* Real depth (core/modules/<mod>/include/<mod>/x.h is five directories deep), two
...
{
    bool ok = true;
    ok = ok && cmp_write(dir, "core/modules/net/include/net/cmp_a.h",
                         "#ifndef CMP_A_H\n#define CMP_A_H\nint cmp_a(void);\n#endif\n");
    ok = ok && cmp_write(dir, "core/modules/net/src/cmp_a.c",
                         "/* cmp_a — proof fixture. */\n#include \"net/cmp_a.h\"\n"
                         "int cmp_a(void)\n{\n    return 1;\n}\n");
    ok = ok && cmp_write(dir, "core/modules/net/src/cmp_b.c",
                         "/* cmp_b — proof fixture. */\nint cmp_b(void)\n{\n"
                         "    return 2;\n}\n");
    ok = ok && cmp_write(dir, "core/modules/crypto/src/cmp_c.c",
                         "/* cmp_c — proof fixture. */\nint cmp_c(void)\n{\n"
                         "    return 3;\n}\n");
...
static int test_cmp_round_trip(void)
{
    ...
    TEST("code_merkle_proof: a leaf, a directory node, the root, and a deep "
        ...
        static const char *const paths[] = {
            "core/modules/net/src/cmp_a.c",          /* an ordinary leaf */
            "core/modules/net",                      /* a directory node */
            "",                             /* the root itself */
            "core/modules/net/include/net/cmp_a.h",  /* the deep path */
        ...
        /* the deep path really is deep: five directories plus the root */
        ASSERT(cmp_prove_ok(m, "core/modules/net/include/net/cmp_a.h", p, &d, &root));
        ASSERT(p->nlevels == 6);
        ASSERT(strcmp(p->level[0].path, "core/modules/net/include/net") == 0);
        ASSERT(strcmp(p->level[1].path, "core/modules/net/include") == 0);
        ASSERT(strcmp(p->level[2].path, "core/modules/net") == 0);
        ASSERT(strcmp(p->level[3].path, "core/modules") == 0);
        ASSERT(strcmp(p->level[4].path, "core") == 0);
        ASSERT(strcmp(p->level[5].path, "") == 0);
        ASSERT(p->kind == CI_MERKLE_KIND_FILE);

        /* a directory node's proof is a directory's proof */
        ASSERT(cmp_prove_ok(m, "core/modules/net", p, &d, &root));
        ASSERT(p->kind == CI_MERKLE_KIND_DIR);

        /* an absent path is an answer, not a failure */
        bool found = true;
        ASSERT(ci_merkle_prove(m, "core/modules/net/src/nope.c", p, &d, &found));
        ASSERT(!found);
...
static int test_cmp_cost_is_sum_of_siblings(void)
{
    ...
    TEST("code_merkle_proof: proof size is the SUM of sibling counts along the "
        ...
        /* narrow path: core/modules/net/src has 2 files; core/modules/net has
         * 2 dirs; core/modules has 2 modules; and core plus root close it. */
        ASSERT(cmp_prove_ok(m, "core/modules/net/src/cmp_a.c", p, &d, &root));
...
static int test_cmp_path_root_kind_binding(void)
{
    ...
    TEST("code_merkle_proof: a proof for path A does not verify for path B, "
        ...
        ASSERT(cmp_prove_ok(m, "core/modules/net/src/cmp_a.c", pa, &da, &root));
        ASSERT(cmp_prove_ok(m, "core/modules/net/src/cmp_b.c", pb, &db, &root));
        ASSERT(cmp_prove_ok(m, "core/modules/net", pd, &dd, &root));
        ...
        ASSERT(mut);
        snprintf(mut->path, sizeof(mut->path), "%s", "core/modules/net/src/cmp_b.c");
        ...
        mut = cmp_dup(pa);
        ASSERT(mut);
        snprintf(mut->path, sizeof(mut->path), "%s", "core/modules/crypto/src/cmp_c.c");
...
static int test_cmp_real_tree(void)
{
    ...
    TEST("code_merkle_proof: a real repository path proves against a real "
        ...
        static const char *const real_paths[] = {
            ...
            "cognition/modules/codeindex/include/codeindex/codeindex_merkle.h",
            "cognition/modules/codeindex",
            "tests/harness/src/test_code_merkle_proof.c",
        };
...
static int test_cmp_canonical_path_inputs(void)
{
    ...
    TEST("code_merkle_proof: a leading slash, a trailing slash, an empty or "
        ...
        ASSERT(cmp_prove_ok(m, "core/modules/net/src/cmp_a.c", pdeep, &ddeep, &root));
        ...
        static const struct {
            ...
            {true,  "/core/modules/net/src/cmp_a.c"},     /* leading slash, deep */
            {true,  "core/modules/net/src/cmp_a.c/"},     /* trailing slash, deep */
            {true,  "lib//net/src/cmp_a.c"},     /* empty segment, deep */
            {true,  "lib/./net/src/cmp_a.c"},    /* dot segment, deep */
            {true,  "core/modules/net/../net/src/cmp_a.c"}, /* parent segment, deep */
        };
...
static int test_cmp_real_tree_worst_case(void)
{
    ...
    TEST("code_merkle_proof: the widest file and directory proofs in this "
         ...
         * directory. This test's own source file lives in tests/harness/src, which
        ...
        static const char *const self = "tests/harness/src/test_code_merkle_proof.c";
        struct zcl_sha3_digest d;
        bool found = false;
        ...
        ASSERT(strcmp(w.dir_path, "tests/harness/src") == 0);
