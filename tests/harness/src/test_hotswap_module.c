 ...
 * engine/modules/hotswap/src/hotswap_activate.c — with -DZCL_DEV_BUILD, so `make
...
/* The status controller row of engine/composition/hotswap_swappable.def, whose declared
 * probe leaf in engine/composition/hotswap_eligible.def is core.status. */
#define STATUS_TU "engine/controllers/src/status_native_handlers.c"
...
static int test_admit_ok(void)
{
    ...
    TEST("hotswap_module_admit accepts a well-formed allowlisted module") {
        struct zcl_hotswap_module m = {
            ...
            .source_tu = STATUS_TU,   /* on engine/composition/hotswap_swappable.def */
            .leaf_count = 1,
...
static int test_admit_allowlist(void)
{
    ...
    TEST("a non-allowlisted source is refused at stage=allowlist (HARD LINE)") {
        struct zcl_hotswap_module m = {
            ...
            .source_tu = "core/modules/consensus/src/pow.c",
...
#define SECT_MAX (ZCL_CORE_SEAL_SECTION_COUNT + 1)
struct sect_fixture {
