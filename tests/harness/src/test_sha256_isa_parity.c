 ...
 *
 * ACCEL-ORACLE: core/modules/crypto/src/sha256.c
 *
 * Differential parity oracle for the two SHA-256 compression transforms in
 * core/modules/crypto/src/sha256.c: the portable C reference and the native hardware
...
int test_sha256_isa_parity(void)
{
    ...
    if (!hw) {
        ...
        printf("  The host reports %s but sha256.c refuses to install it.\n"
               ...
               "  preprocessor one — see core/modules/crypto/src/sha256.c.\n",
               hardware_tier_name());
        failures++;
    }
