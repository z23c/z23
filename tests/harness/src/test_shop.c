 ...
 * contexts/market/controllers/src/shop_native_handler.c).
 *
 * Covered:
 ...
 *      "shop" into <datadir>/directory/apps.csv, deduped, and core/modules/net's
...
/* ── 6. the directory announcement round-trips through core/modules/net ──────── */
static int shop_directory_announcement(void)
{
    ...
    SH_CHECK("core/modules/net reads \"shop\" back from the apps file",
             onion_directory_extra_apps_csv(dir, csv, sizeof(csv)) > 0 &&
             strcmp(csv, "shop") == 0);
