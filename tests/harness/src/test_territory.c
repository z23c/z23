 ...
 *
 * test_territory — the cognition/modules/territory gate.
 *
 ...
 * than a magic number: one names platform/modules/base (and owes it a baseline row), one
 * names core/modules/net only, and one names no territory path at all. */
...
static const char *GATE_BASE_SH =
    ...
    "grep -rn alpha platform/modules/base/src/ || exit 1\n";

static const char *GATE_NET_SH =
    "#!/bin/sh\n"
    "grep -rn call core/modules/net/src/ || exit 1\n";
...
static const char *GATE_BASE_BASELINE =
    "# grandfathered rows\n"
    "platform/modules/base/src/alpha.c alpha_dead 120\n";
...
int test_territory(void)
{
    ...
    test_rm_rf(TERR_FIX);
    if (!terr_write(TERR_FIX, "platform/modules/base/include/base/alpha.h", ALPHA_H) ||
        !terr_write(TERR_FIX, "platform/modules/base/include/base/beta.h", BETA_H) ||
        !terr_write(TERR_FIX, "platform/modules/base/src/alpha.c", ALPHA_C) ||
        !terr_write(TERR_FIX, "platform/modules/base/src/registry.c", REGISTRY_C) ||
        !terr_write(TERR_FIX, "core/modules/net/src/caller.c", CALLER_C) ||
        !terr_write(TERR_FIX, "tests/harness/src/test_fixgroup.c", TEST_ENTRY_C) ||
    ...
    TEST("territory_list: names the fixture's declared modules") {
        ...
        for (int i = 0; i < n; i++) {
            if (strcmp(names[i], "platform/modules/base") == 0) saw_base = true;
            if (strcmp(names[i], "core/modules/net") == 0) saw_net = true;
        }
        ASSERT(saw_base);
        ASSERT(saw_net);
        PASS();
    ...
    r = territory_scorecard(ci, TERR_FIX, "platform/modules/base", rs, NULL);
    ASSERT(r != NULL);

    TEST("scorecard: owns exactly the fixture's platform/modules/base files") {
        ASSERT(r->found);
    ...
    TEST("scorecard: a symbol a test really calls is REACHED") {
        ...
        ASSERT_STR_EQ(s->header, "platform/modules/base/include/base/alpha.h");
        PASS();
    }
    ...
    r = territory_scorecard(ci, TERR_FIX, "platform/modules/base", NULL, NULL);
    ASSERT(r != NULL);
    ...
    TEST("empty catalog: nothing is reached, and nothing pretends to be") {
        ...
        struct territory_report *r2 =
            territory_scorecard(ci, TERR_FIX, "platform/modules/base", empty, NULL);
        ASSERT(r2 != NULL);
    ...
    b = territory_brief_build(ci, TERR_FIX, "platform/modules/base", rs, &frouter, gates,
                              NULL);
    ASSERT(b != NULL);
    ...
    TEST("brief: absent gate wiring reports unreadable, not zero gates") {
        ...
        struct territory_brief *nb =
            territory_brief_build(ci, TERR_FIX, "platform/modules/base", rs, &frouter,
                                  none_g, NULL);
        ASSERT(nb != NULL);
