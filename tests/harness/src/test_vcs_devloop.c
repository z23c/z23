 ...
 * (contexts/commons/modules/vcs/src/vcs_devloop.c), the function tools/dev/devloop_cycle.c:
    ...
    *policy = (struct vcs_zcode_proof_policy_v1) {
    ...
    bool task_authority =
        ...
        vcs_package_recipe_add_source(
            &recipe, "engine/entry/main.c", &recipe_error);
...
static int t_anchor_ok(const char *dir)
{
    ...
    vd_write(dir, "engine/entry/main.c", "int main(void){return 0;}\n");
...
/* contexts/commons/modules/vcs never spawns a process to run the initial baseline (ZVCS
...
static int t_initial_anchor_deferred(const char *dir)
{
     ...
     * held, since contexts/commons/modules/vcs never launches anything on its own). */
...
static int t_publication_enqueue(const char *dir)
{
    ...
    vd_write(dir, "engine/entry/main.c", "int main(void){return 1;}\n");
