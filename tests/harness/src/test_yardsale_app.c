 ...
 *
 * test_yardsale_app — the yardsale MVC app (contexts/commons/apps/yardsale/app.def): the
 * manifest registration, and the swap ceremony driven end-to-end THROUGH
 * the controller functions (contexts/market/controllers/src/yardsale_controller.c)
...
static int t_seller_web_endpoint(void)
{
    ...
    YSA_CHECK("web: accept POST is 200 octet-stream",
              ...
              strstr((const char *)response,
                     "engine/application/octet-stream") != NULL);
