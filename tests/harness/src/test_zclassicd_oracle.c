...
static void *mock_server_loop(void *arg)  /* raw-pthread-ok: test-local */
{
    ...
    while (!atomic_load(&m->stop)) {
        ...
        buf[0] = '\0';
        size_t got = 0;
        /* 5s to match LRC_TIMEOUT_SECS in engine/modules/rpc/src/legacy_rpc_client.c;
...
static void mock_server_stop(struct mock_server *m)
{
    ...
    /* Shut down before close so a blocking accept() is reliably interrupted
     * on Linux as well as Windows.  A close from another thread alone can
     * leave accept() holding its own reference to the socket until another
     * connection arrives. */
