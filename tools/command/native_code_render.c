...
/* code.map's first pass: the exact maintained roots, shared with every
 * whole-tree scanner. Each count is aggregate so lib/ and app/ include their
 * child groups. Returns false (capacity exceeded) when more than
 * CODE_MAP_ROOT_CAP roots have files — the caller fails the reply. */
static bool code_map_render_roots(struct codeindex *ci,
                                  const struct ci_group *groups, int ng,
                                  struct json_value *roots, int *total,
                                  int *nroot)
{
...
#undef SOURCE_ROOT
    ...
    *total = 0; *nroot = 0;
    for (size_t i = 0;
        ...
        if (fc <= 0) continue;
        if (*nroot >= CODE_MAP_ROOT_CAP) return false;
        *total += fc;
...
/* code.map's second pass: product shapes recur inside feature rooms and the
 * engine. Aggregate their direct counts without inventing a second physical
 * `app/` tree. */
static void code_map_render_shapes(struct codeindex *ci,
                                   const struct ci_group *groups, int ng,
                                   struct json_value *shapes, int *nshape)
{
    size_t nsh = 0;
...
/* code.map's cross-check: the source-kind totals (from the codeindex's own
 * counting pass) must agree with the roots/shapes totals derived above.
 * Returns NULL when consistent, else the (code, reason) pair to fail with. */
static const char *const *code_map_counts_disagreement(
    const struct ci_source_file_counts *counts, int total)
{
    static const char *const disagreement[] = {
        "COUNT_DISAGREEMENT",
        "code map source-kind totals disagree with roots",
    };
    static const char *const governed_disagreement[] = {
        "GOVERNED_COUNT_DISAGREEMENT",
                                ...
                                struct zcl_command_reply *reply)
{
    ...
    int total = 0, nroot = 0, nshape = 0;
    bool roots_ok = code_map_render_roots(ci, groups, ng, &roots, &total,
                                          &nroot);
    if (!roots_ok) {
    ...
    code_map_render_shapes(ci, groups, ng, &shapes, &nshape);
    ...
    const char *const *disagreement =
        code_map_counts_disagreement(&counts, total);
    if (disagreement) {
        json_free(&roots); json_free(&shapes);
        codeindex_close(ci);
        zcl_command_reply_fail(reply, ZCL_COMMAND_STATUS_FAILED,
                               ZCL_COMMAND_EXIT_INTERNAL,
                               disagreement[0], "verify", false, false,
                               disagreement[1], "");
        return;
    }
...
/* code.room's feature-room commands[] pass: every registered command path
 * under `root`, capped. Returns the true count (possibly over the cap). */
static int code_room_collect_commands(const struct zcl_command_registry *registry,
                                      const char *root,
...
/* The handler-symbol/implementation-file join, bundled: every distinct
 * registered handler symbol under `root` that resolves to an indexed def
 * file, and every distinct def file those handlers land in. */
struct code_room_handlers {
    char files[CODE_ROOM_FEATURE_FILE_CAP][256];
    const char *names[CODE_ROOM_FEATURE_HANDLER_CAP];
    int file_count, name_count, total, unindexed;
    bool name_truncated, file_truncated;
};

static void code_room_collect_handlers(
    struct codeindex *ci, const char *root,
    const struct zcl_command_handler_index *handlers,
    struct code_room_handlers *h)
{
    *h = (struct code_room_handlers){0};
    for (size_t i = 0; handlers && i < handlers->count; i++) {
        ...
        for (int j = 0; j < h->name_count; j++)
            if (strcmp(h->names[j], entry->handler_name) == 0)
                seen_handler = true;
        if (!seen_handler) {
            if (h->name_count < CODE_ROOM_FEATURE_HANDLER_CAP)
                h->names[h->name_count++] = entry->handler_name;
            else
                h->name_truncated = true;
        }
        if (!code_room_has_file(h->files, h->file_count, symbol.def_path)) {
            if (h->file_count < CODE_ROOM_FEATURE_FILE_CAP) {
                (void)snprintf(h->files[h->file_count],
                               sizeof(h->files[h->file_count]), "%s",
                               symbol.def_path);
                h->file_count++;
            } else {
...
/* The distinct source groups the handler implementation files above land in. */
static int code_room_collect_groups(struct codeindex *ci,
                                    char handler_files[][256],
                                    int handler_file_count,
...
/* The shared-rule test-group union over the handler implementation files,
 * and whether any of them is a consensus surface. */
static size_t code_room_collect_tests(char handler_files[][256],
                                      int handler_file_count,
                                      ...
                                      bool *consensus_risk)
{
    ...
    *consensus_risk = false;
    ...
    return proofs.groups_len;
}

/* Commands OUTSIDE this room whose registered handler lands in one of this
 * room's own implementation files — the mixed-file coupling this leaf exists
 * to surface. */
static int code_room_collect_coupled(
    struct codeindex *ci, const struct zcl_command_handler_index *handlers,
    const char *root, char handler_files[][256], int handler_file_count,
...
/* code.room feature-room reply, part 1: identity, commands, handler symbols,
 * and the implementation file/group listing. */
static void code_room_feature_emit_identity(
    struct zcl_command_reply *reply, const char *root,
    const char *purpose, struct json_value *commands, int command_total,
    const struct code_room_handlers *h, struct json_value *handler_arr,
    struct json_value *file_arr, struct json_value *group_arr,
    int group_count)
{
    ...
    (void)json_push_kv_str(&reply->data, "purpose", purpose ? purpose : "");
    ...
    (void)json_push_kv_int(&reply->data, "handler_count", h->name_count);
    (void)json_push_kv_int(&reply->data, "handler_binding_count", h->total);
    (void)json_push_kv_int(&reply->data, "handler_unindexed", h->unindexed);
    ...
    (void)json_push_kv_bool(&reply->data, "implementation_complete",
                            !h->file_truncated && h->unindexed == 0);
}

/* code.room feature-room reply, part 2: test groups, consensus risk, the
 * cross-command coupling list, and the closing summary line. */
static void code_room_feature_emit_proof(
    struct zcl_command_reply *reply, const char *root, int command_total,
    const struct code_room_handlers *h, struct json_value *test_arr,
    size_t test_groups_len, bool consensus_risk, struct json_value *coupled,
    int coupled_total)
{
    (void)json_push_kv(&reply->data, "test_groups", test_arr);
    (void)json_push_kv_bool(&reply->data, "test_groups_truncated",
                            test_groups_len > (size_t)CODE_TESTS_CAP);
    ...
    (void)snprintf(summary, sizeof(summary),
                   ...
                   root, command_total, h->file_count, test_groups_len,
                   coupled_total);
    (void)json_push_kv_str(&reply->data, "summary", summary);
}

/* code.room feature-room's handler_symbols[]/implementation_files[] arrays:
 * a straight line-render of the already-collected handler names/files. */
static void code_room_render_handler_arrays(const struct code_room_handlers *h,
                                            struct json_value *handler_arr,
                                            struct json_value *file_arr)
{
    for (int i = 0; i < h->name_count; i++)
        code_push_line(handler_arr, h->names[i]);
    for (int i = 0; i < h->file_count; i++)
        code_push_line(file_arr, h->files[i]);
}
    ...
    struct codeindex *ci, const char *root)
{
    ...
    int command_total = code_room_collect_commands(registry, root, &commands);
    ...
    struct code_room_handlers h;
    code_room_collect_handlers(ci, root, handlers, &h);
    ...
    code_room_render_handler_arrays(&h, &handler_arr, &file_arr);

    struct json_value group_arr;
    json_init(&group_arr); json_set_array(&group_arr);
    int group_count =
        code_room_collect_groups(ci, h.files, h.file_count, &group_arr);

    struct json_value test_arr;
    json_init(&test_arr); json_set_array(&test_arr);
    bool consensus_risk = false;
    size_t test_groups_len = code_room_collect_tests(
        h.files, h.file_count, &test_arr, &consensus_risk);

    struct json_value coupled;
    json_init(&coupled); json_set_array(&coupled);
    int coupled_total = code_room_collect_coupled(ci, handlers, root, h.files,
                                                  h.file_count, &coupled);

    code_room_feature_emit_identity(reply, root,
                                    root_spec->summary, &commands,
                                    command_total, &h, &handler_arr,
                                    &file_arr, &group_arr, group_count);
    code_room_feature_emit_proof(reply, root, command_total, &h, &test_arr,
                                 test_groups_len, consensus_risk, &coupled,
                                 coupled_total);
...
/* neighbors: sibling files stamped with EXACTLY this `group`, `path` itself
 * excluded, capped at CODE_ROOM_NEIGHBOR_CAP. `*gcount` is the accurate group
 * size (from the count query), so the caller's neighbors_truncated stays
 * exact even past the render cap; returns the number actually rendered. */
static int code_room_render_neighbors(struct codeindex *ci, const char *group,
                                      const char *path,
                                      struct json_value *neigh, int *gcount)
{
    int shown = 0;
    *gcount = 0;
    if (!group[0]) return 0;
...
/* code.room's context/shape block: the classification, its matches array,
 * and the file's group/purpose. */
static void code_room_emit_context(
    struct zcl_command_reply *reply, const char *path, bool ffound,
    const struct ci_context_assignment *assignment, const char *purpose,
    const char *group)
{
...
/* code.room's neighbors[] block: sibling files stamped with EXACTLY this
 * group, this file excluded, capped. Returns the number rendered and, via
 * `gcount`, the accurate group size (so neighbors_truncated stays exact past
 * the render cap). */
static int code_room_emit_neighbors(struct zcl_command_reply *reply,
                                    struct codeindex *ci, const char *group,
                                    const char *path, int *gcount)
{
    struct json_value neigh;
    json_init(&neigh); json_set_array(&neigh);
    int shown = code_room_render_neighbors(ci, group, path, &neigh, gcount);
...
/* code.room's commands[] block: WF4 4C dispatch-index join — command paths
 * whose registered native handler is DEFINED in this file. Empty is a real
 * answer (no leaf's handler lives here), never a guess. */
static void code_room_emit_commands(struct zcl_command_reply *reply,
                                 ...
                                 struct zcl_command_reply *reply)
{
    ...
    code_room_emit_context(reply, path, ffound, &assignment,
                           ffound ? finfo.purpose : "", group);
    ...
    int gcount = 0;
    int shown = code_room_emit_neighbors(reply, ci, group, path, &gcount);
    ...
    code_room_emit_commands(reply, ci, path);
...
/* `general`'s brief "owns" section: fixed counters off the territory report,
 * no branches — its own function purely to keep the caller's decision count
 * down, not because it has any of its own. */
static void general_brief_emit_owns(struct zcl_command_reply *reply,
...
/* `general`'s brief "proves" section: routed and reached, never summed. */
static void general_brief_emit_proves(struct zcl_command_reply *reply,
                                      const struct territory_report *r)
{
...
/* `general`'s brief "trusts" section: the declared reproducibility hole,
 * never omitted. */
static void general_brief_emit_trusts(struct zcl_command_reply *reply,
                                      const struct territory_brief *b)
{
...
/* `general`'s brief "refuses" section: the three buckets that partition the
 * wired lint gates. */
static void general_brief_emit_refuses(struct zcl_command_reply *reply,
                                       const struct territory_brief *b)
{
...
/* `general`'s brief "weak_at" section: unreached/unknown public symbols and
 * unrouted files, plus the two declared holes (unproven, duplicate_clusters). */
/* `general`'s weak_at symbols/files rows: unreached, unknown (with reason),
 * and unrouted files, each capped. Pushed onto the `weak` object directly so
 * the caller does not also need each array handle. */
static void general_brief_weak_render_rows(struct json_value *weak,
                                           const struct territory_report *r)
{
    struct json_value wsym, wunk, wfile;
...
static void general_brief_emit_weak_at(struct zcl_command_reply *reply,
                                       const struct territory_report *r,
                                       const struct territory_brief *b)
{
    struct json_value weak;
    json_init(&weak); json_set_object(&weak);
    general_brief_weak_render_rows(&weak, r);
...
/* `general`'s brief "depends" section: what this territory depends on, and
 * what depends on it. */
static void general_brief_emit_depends(struct zcl_command_reply *reply,
                                       const struct territory_report *r)
{
...
/* `general`'s brief "cost" section: what this call cost, per phase. */
static void general_brief_emit_cost(struct zcl_command_reply *reply,
                                    const struct territory_report *r,
                                    const struct territory_brief *b,
                                    const struct territory_reach_set *rs)
{
                                 ...
                                 struct zcl_command_reply *reply)
{
    ...
    general_brief_emit_owns(reply, r);
    general_brief_emit_proves(reply, r);
    general_brief_emit_trusts(reply, b);
    general_brief_emit_refuses(reply, b);
    general_brief_emit_weak_at(reply, r, b);
    general_brief_emit_depends(reply, r);
    general_brief_emit_cost(reply, r, b, rs);
