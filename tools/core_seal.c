 ...
 *
 * core_seal — the owner-gated byte-integrity manifest tool (Wave 1.1 / W0).
 *
 * The Makefile selects the sealed set through CORE_SEAL_PATHS. In the current
 * tree that is every tracked path below core/ except the manifest itself;
 * core/ includes reusable protocol/runtime modules as well as the pure
 * consensus contexts. This tool classifies none of those files semantically.
 * It pins their bytes in core/MANIFEST.sha3: one per-file SHA3-256 digest plus
 * a ROOT over the sorted (path, filehash) stream. Any change to a selected file
 * changes ROOT, which `make core-seal-check` catches.
 *
 ...
 * are NOT identical (core/UNSEAL.md is sealed but not indexed; the explicitly
 * named validation pathspecs are already below the core/ glob), so the
 * WHOLE-TREE roots differ by construction. A directory node's digest is
 * comparable between the two trees exactly when that directory's file set is
 * identical in both — which is the measurement that decides whether this seal
 * can become a pure view over the codeindex tree.
 *
 ...
 * sealed set itself comes only from CORE_SEAL_PATHS in the Makefile and
 * `git ls-files`; derive its count instead of copying one here. No input is
 * selected by mtime, inode, or an untracked filesystem walk. The one sealed
 * file any tooling writes is core/UNSEAL.md, and only the owner-run
 * `make core-unseal` ritual writes it — immediately before the `make core-seal`
 * that re-freezes it.
 *
...
static const char *const manifest_header =
    "# Owner-gated byte-integrity seal — SHA3-256 manifest. "
    "AUTO-GENERATED by `make core-seal`.\n"
    "# Do not edit by hand. `make core-seal-check` fails loud if any sealed "
    "file drifts from this.\n"
    "# Scope comes only from CORE_SEAL_PATHS in the Makefile. In this tree it\n"
    "# selects every tracked path below core/ except this manifest; core/\n"
    "# includes reusable protocol/runtime modules as well as pure consensus\n"
    "# contexts. The seal proves byte identity, not semantic classification.\n"
    "# See docs/adr/0002-sealed-consensus-core.md.\n"
    "#\n"
