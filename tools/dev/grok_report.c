...
static void print_value(const struct json_value *v)
{
    ...
    char *buf = malloc(need + 1); // raw-alloc-ok:standalone-dev-cli
    if (!buf) {
...
int main(int argc, char **argv)
{
    ...
    char *text = malloc(cap); // raw-alloc-ok:standalone-dev-cli
    if (!text) {
        fclose(f);
    ...
    for (;;) {
        if (len == cap) {
            size_t want = cap * 2;
            char *bigger = realloc(text, want); // raw-alloc-ok:standalone-dev-cli
            if (!bigger) {
                free(text);
                fclose(f);
