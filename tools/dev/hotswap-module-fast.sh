...
if [ "$(find contexts/explorer/views/templates contexts/explorer/views/css -type f -newer "$VH1" 2>/dev/null | head -1)" ]; then
    fallback "view template newer than $VH1"
fi
[ contexts/explorer/views/src/site.css -nt "$VH2" ] && fallback "site.css newer than $VH2"
