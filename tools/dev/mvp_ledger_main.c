...
"            input_tokens cache_creation_tokens cache_read_tokens outcome.\n"
...
"            verifier_rounds landed. Agents join a loop by lane name; a\n"
"            train assembler is split evenly across the loops whose lane is\n"
"            on that train, a design workflow across the loops whose\n"
"            evidence names it.\n"
...
"            landed_loops tokens_raw tokens_out tcu loops_per_mtcu\n"
"            tcu_per_loop note.\n"
...
"  --ancestry <file>    one `git rev-list <origin/main>` output; without it\n"
"                       the landed column stays '-' (not asked)\n"
...
"file, the line number and the reason.\n", stdout);
}
...
static int mvl_mode_agents(const struct mvl_opts *o)
{
    ...
    if (!mvl_agents_alloc(&agents))
        return 2;
    ok = mvl_scan_session(o->session, &agents, err, sizeof err);
    if (ok) {
...
static bool mvl_load_ancestry(const struct mvl_opts *o,
                              char (**anc)[MVL_ID_CAP], size_t *count,
                              char *err, size_t err_cap)
{
    ...
    *anc = zcl_calloc(MVL_MAX_ANCESTRY, MVL_ID_CAP, "mvl_ancestry");
    ...
    return mvl_read_ancestry(o->ancestry, *anc, MVL_MAX_ANCESTRY, count, err,
                             err_cap);
}
...
static int mvl_mode_loops(const struct mvl_opts *o)
{
    ...
    if (ok) {
        mvl_join_loops(&plan, &agents, o->trains, anc, anc_count, joins);
...
/* Collects the lanes a verifier landed at or after t0, for the KPI
 * numerator. `--since` is t0; without it every VERDICT counts. */
static bool mvl_load_verified(const struct mvl_opts *o,
                              char (**lanes)[MVL_LANE_CAP], size_t *count,
                              char *err, size_t err_cap)
{
    ...
    *lanes = zcl_calloc(MVL_MAX_TRAIN_LANES, MVL_LANE_CAP, "mvl_lanes");
    ...
    if (o->since && !mvl_parse_timestamp(o->since, &t0)) {
        (void)snprintf(err, err_cap,
                       "--since:0: mvl_bad_timestamp: %s", o->since);
        return false;
    }
    mvl_scratch_dir(o, scratch, sizeof scratch);
    return mvl_verified_lanes(scratch, t0, *lanes, MVL_MAX_TRAIN_LANES, count,
                              err, err_cap);
}
...
static int mvl_mode_kpi(const struct mvl_opts *o)
{
    ...
    ok = mvl_load(o, &plan, &agents, err, sizeof err);
    if (ok)
        ok = mvl_load_verified(o, &lanes, &lane_count, err, sizeof err);
    if (ok)
        ok = mvl_load_ancestry(o, &anc, &anc_count, err, sizeof err);
    if (ok) {
                              ...
                              char *err, size_t err_cap)
{
    ...
    ok = mvl_scan_session(o->session, &agents, err, err_cap);
    if (ok) {
        mvl_apply_since(&agents, o->since);
