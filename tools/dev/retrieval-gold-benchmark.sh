...
commit_subject_tasks=0; same_commit_tasks=0
...
while IFS= read -r row || [[ -n $row ]]; do
    ...
    provenance=$(field "$row" query_provenance)
    case "$provenance" in
        commit_subject_only) commit_subject_tasks=$((commit_subject_tasks + 1)) ;;
        same_commit_unordered_*) same_commit_tasks=$((same_commit_tasks + 1)) ;;
        *) fail "unknown query provenance for $id: $provenance" ;;
    esac
...
[[ $tasks -eq $declared_tasks &&
   ...
   $((commit_subject_tasks + same_commit_tasks)) -eq $declared_tasks ]] ||
