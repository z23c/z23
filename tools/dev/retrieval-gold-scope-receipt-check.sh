...
# Verify the frozen ten-task publishable retrieval benchmark with its
# post-ranking directory-group scope proxy, without rerunning ranking.
...
readonly driver=705d16ccab6b9732c81f95159627eef18fb05a08
readonly receipt_name=retrieval-gold-benchmark-705d16ccab6b.jsonl
readonly receipt="$repo_root/docs/work/retrieval-gold-evidence/$receipt_name"
readonly receipt_sha3=50c1adf1fdddeb198b0da056dbc8a091ea549c73b449a963467315874f3f8b4b
...
readonly observed_task_v3_keys='record,schema,id,status,expected_vcs_root,shared_codeindex_source_root_sha3,membership_tree_root_sha3,membership_join_basis,pages,ranking_compute,elapsed_us,budget_ms,budget_exceeded,all_pages,wall_us,single_process,buffered_before_write,literal,retained_files,ranking_complete,ranking_root_sha3,projected_context_bytes_at_5,approximate_tokens_at_5,bm25,retained_files,ranking_complete,ranking_root_sha3,projected_context_bytes_at_5,approximate_tokens_at_5,scope_available,scope_basis,scope_interpretation,scope_classifier_epoch,files_read_observed,reuse_success_available,unique_loc_avoided_available'
...
fail() { printf 'retrieval-gold-scope-receipt-check: FAIL — %s\n' "$*" >&2; exit 1; }
...
validate_semantics() {
    ...
    for ((i = 0; i < 10; i++)); do
        ...
        [[ $row_schema = zcl.retrieval_gold_benchmark_task.v3 ]] ||
            fail "task receipt is not v3 at record $((i + 2))"
        ...
        keys_exact "$row" . "$observed_task_v3_keys"
        ...
        [[ $(bool_field "$row" all_pages.single_process) = true &&
           $(bool_field "$row" all_pages.buffered_before_write) = true ]] ||
            fail "v3 all-page transport boundary differs: $id"
    ...
    [[ $(root_field "$benchmark" evaluator_batch_root_sha3) = abd5b8845eaa99ab67966d0050e962df1c44598e8c35e6613a397818ff21c250 ]] ||
    ...
    awk -v out="$tmp" -v task_map="$rank_task_map" '
        function refuse(message) {
            print "retrieval-gold-scope-receipt-check: batch rank extraction: " message > "/dev/stderr"
            bad = 1
            exit 1
        }
        function finish_arm() {
    ...
    [[ $(field "$aggregate" record) = aggregate &&
       $(field "$aggregate" schema) = zcl.retrieval_gold_benchmark_aggregate.v2 &&
...
selftest() {
    ...
    sed -n \
        '1s|^.*"evaluator_batch_encoding":"base64_rfc4648","evaluator_batch_base64":"\([A-Za-z0-9+/]*=\{0,2\}\)","evaluator_batch_root_sha3":"[0-9a-f]\{64\}"}$|\1|p' \
        "$receipt" | tr -d '\n' | decode_base64 >"$tmp/evaluator.batch" ||
        fail "selftest could not decode the sealed evaluator batch"
    [[ $(hash_file "$tmp/evaluator.batch") = abd5b8845eaa99ab67966d0050e962df1c44598e8c35e6613a397818ff21c250 ]] ||
        fail "selftest decoded a different evaluator batch"
    bad="$tmp/top-five-scope.batch"
    sed '0,/^rank \([0-9][0-9]*\) 1 [01] /s//rank \1 0 0 /' \
        "$tmp/evaluator.batch" >"$bad"
    if (validate_batch_scope "$bad" "$tmp/top-five-scope.counts" >/dev/null 2>&1); then
        fail "top-five missing-scope mutation was accepted"
    fi
    mutations=$((mutations + 1))
    bad="$tmp/post-five-scope.batch"
    awk 'BEGIN { OFS=" " } $1 == "rank" { seen++; if (seen == 6) { $3=1; $4=0 } } { print }' \
        "$tmp/evaluator.batch" >"$bad"
    if (validate_batch_scope "$bad" "$tmp/post-five-scope.counts" >/dev/null 2>&1); then
    ...
    bad="$tmp/driver.jsonl"; sed 's/705d16ccab6b9732c81f95159627eef18fb05a08/705d16ccab6b9732c81f95159627eef18fb05a09/' "$receipt" >"$bad"
    ...
    bad="$tmp/batch-root.jsonl"; sed 's/abd5b8845eaa99ab67966d0050e962df1c44598e8c35e6613a397818ff21c250/0bd5b8845eaa99ab67966d0050e962df1c44598e8c35e6613a397818ff21c250/' "$receipt" >"$bad"
    expect_semantic_refusal batch-root "$bad"; mutations=$((mutations + 1))
    bad="$tmp/historical-binary-root.jsonl"
    sed 's/417159ed6c2dde9162141a804a588c2a05b04d5a39844e553e1467fa775dafad/g17159ed6c2dde9162141a804a588c2a05b04d5a39844e553e1467fa775dafad/' \
        "$receipt" >"$bad"
    ...
    bad="$tmp/task-scope-available.jsonl"; sed '2s/"scope_available":true/"scope_available":false/' "$receipt" >"$bad"
    expect_semantic_refusal task-scope-available "$bad"; mutations=$((mutations + 1))
    bad="$tmp/task-scope-basis.jsonl"; sed '2s/reviewed_relevant_codeindex_group_membership_v1/reviewed_relevant_codeindex_group_membership_v0/' "$receipt" >"$bad"
    expect_semantic_refusal task-scope-basis "$bad"; mutations=$((mutations + 1))
    bad="$tmp/task-scope-interpretation.jsonl"; sed '2s/directory_taxonomy_proxy_not_semantic_scope/directory_taxonomy_proxy/' "$receipt" >"$bad"
    expect_semantic_refusal task-scope-interpretation "$bad"; mutations=$((mutations + 1))
    bad="$tmp/task-scope-epoch.jsonl"; sed '2s/current_driver_over_exact_parent_source/current_driver_over_current_source/' "$receipt" >"$bad"
    expect_semantic_refusal task-scope-epoch "$bad"; mutations=$((mutations + 1))
    bad="$tmp/aggregate-schema.jsonl"; sed '$s/zcl.retrieval_gold_benchmark_aggregate.v2/zcl.retrieval_gold_benchmark_aggregate.v1/' "$receipt" >"$bad"
    expect_semantic_refusal aggregate-schema "$bad"; mutations=$((mutations + 1))
    bad="$tmp/aggregate-scope-basis.jsonl"; sed '$s/reviewed_relevant_codeindex_group_membership_v1/reviewed_relevant_codeindex_group_membership_v0/' "$receipt" >"$bad"
    expect_semantic_refusal aggregate-scope-basis "$bad"; mutations=$((mutations + 1))
    bad="$tmp/aggregate-scope-interpretation.jsonl"; sed '$s/directory_taxonomy_proxy_not_semantic_scope/directory_taxonomy_proxy/' "$receipt" >"$bad"
    expect_semantic_refusal aggregate-scope-interpretation "$bad"; mutations=$((mutations + 1))
    bad="$tmp/aggregate-scope-epoch.jsonl"; sed '$s/current_driver_over_exact_parent_source/current_driver_over_current_source/' "$receipt" >"$bad"
    expect_semantic_refusal aggregate-scope-epoch "$bad"; mutations=$((mutations + 1))
    bad="$tmp/aggregate-scope-aggregation.jsonl"; sed '$s/micro_task_file_selections_at_5/macro_equal_task_weight/' "$receipt" >"$bad"
    expect_semantic_refusal aggregate-scope-aggregation "$bad"; mutations=$((mutations + 1))
    bad="$tmp/aggregate-scope-denominator.jsonl"; sed '$s/sum_of_task_unique_file_selections_at_5/tasks_times_five/' "$receipt" >"$bad"
    expect_semantic_refusal aggregate-scope-denominator "$bad"; mutations=$((mutations + 1))
    bad="$tmp/literal-wrong-scope.jsonl"; sed '$s/"wrong_scope_at_5":{"available":true,"basis_points":7209}/"wrong_scope_at_5":{"available":false,"basis_points":null}/' "$receipt" >"$bad"
    expect_semantic_refusal literal-wrong-scope "$bad"; mutations=$((mutations + 1))
    bad="$tmp/bm25-wrong-scope.jsonl"; sed '$s/"wrong_scope_at_5":{"available":true,"basis_points":4888}/"wrong_scope_at_5":{"available":true,"basis_points":4889}/' "$receipt" >"$bad"
    expect_semantic_refusal bm25-wrong-scope "$bad"; mutations=$((mutations + 1))
    ...
    sed '0,/zcl.retrieval_gold_benchmark_task.v3/s//zcl.retrieval_gold_benchmark_task.v2/' \
        "$receipt" >"$bad"
    expect_semantic_refusal mixed-task-schema "$bad"; mutations=$((mutations + 1))
    printf 'retrieval-gold-scope-receipt-check: SELFTEST PASS mutations=%s\n' "$mutations"
}

cleanup() { [[ -z ${tmp:-} ]] || rm -r -- "$tmp"; }
tmp=$(mktemp -d "${TMPDIR:-/tmp}/z23-retrieval-scope-receipt-check.XXXXXX")
...
case ${1:---check} in
    --check)
        ...
        validate_canonical
        printf 'retrieval-gold-scope-receipt-check: PASS records=12 tasks=10 evaluated=9 unsupported=1 relevance=43 wrong_scope_proxy=directory_taxonomy maintained_evaluator_replay=true historical_binary_identity=observation_unverified receipt_sha3=%s\n' "$receipt_sha3" ;;
    --selftest)
