...
static bool append(struct wide_buffer *buffer, wchar_t value)
{
    if (buffer->used == buffer->capacity) {
        ...
        wchar_t *grown = realloc(buffer->data, next * sizeof(*grown)); /* raw-alloc-ok:standalone-windows-helper */
...
int wmain(int argc, wchar_t **argv)
{
    ...
    startup.lpAttributeList = malloc(attribute_size); /* raw-alloc-ok:standalone-windows-helper */
