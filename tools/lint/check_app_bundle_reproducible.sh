...
#
#   check_app_bundle_reproducible.sh --bin build/bin/zdemo \
#       --probe-args '--frames=12 --quiet' --probe-expect 'selftest: OK frames=12'
#
# Only the subject changes; A-C are byte-for-byte the same proof, and the
# launch probe keeps both halves of its contract: exit 0 (a kernel signature
# rejection kills, never exits) and, when --probe-expect is given, that
# string must appear in stdout. Without --probe-expect the probe still
# demands non-empty stdout. The DEFAULT mode's exact full-output equality is
# deliberately unchanged — this flag loosens nothing that already ran.
#
# ── SELFTEST ────────────────────────────────────────────────────────────────
...
#   decoration.
#
# Mode: default | --selftest | --bin <path> [--app-name <n>] [--probe-args <a>]
#                [--probe-expect <s>].
...
# Set only in --bin mode: the prebuilt binary under proof, and the probe that
# must launch it. Empty SUBJECT_BIN means the default jsonq subject.
SUBJECT_BIN=""
PROBE_ARGS=""
...
PROBE_OUT=""
PROBE_RC=""
# Run the probe: the bundled executable must exit 0 and satisfy the output
# contract — exact full-output equality in the default jsonq mode, a required
# substring (or any non-empty output) in --bin mode. Input comes from a file,
# never a pipe: a pipeline whose exit status is a decision is the trap
# sh_str.sh exists for (A3), and a kernel signature rejection shows up here as
# a signal, not an exit code.
probe_ok() {
    local exe="$1" json_file="$2"
    PROBE_OUT=""
    PROBE_RC=""
    if [ -n "$SUBJECT_BIN" ]; then
        # $PROBE_ARGS is split on purpose: it is a flag list like
        # '--frames=12 --quiet', not one argument.
        if PROBE_OUT="$("$exe" $PROBE_ARGS < /dev/null)"; then
            PROBE_RC=0
        else
            PROBE_RC=$?
        fi
    ...
    if [ -n "$SUBJECT_BIN" ]; then
        if [ -n "$PROBE_EXPECT" ]; then
            case "$PROBE_OUT" in
                *"$PROBE_EXPECT"*) return 0 ;;
            esac
            echo "$GATE_NAME: FAIL — bundled $SUBJECT_APP_NAME output did not contain '$PROBE_EXPECT' (last line: '${PROBE_OUT##*$'\n'}')" >&2
            return 1
        fi
        if [ -z "$PROBE_OUT" ]; then
            echo "$GATE_NAME: FAIL — bundled $SUBJECT_APP_NAME printed nothing; the probe cannot tell a launch from a silent crash" >&2
            return 1
        fi
        return 0
    fi
...
prove_two_runs() {
    ...
    if [ -n "$SUBJECT_BIN" ]; then
        # Prebuilt subject: the same binary goes into both runs, so A below
        # degenerates to a copy check and the proof is entirely about the
        # bundler — which is the point.
        cp "$SUBJECT_BIN" "$run_a/$SUBJECT_APP_NAME" ||
            die "could not copy the subject binary"
        cp "$SUBJECT_BIN" "$run_b/$SUBJECT_APP_NAME" ||
            die "could not copy the subject binary"
        chmod 0755 "$run_a/$SUBJECT_APP_NAME" "$run_b/$SUBJECT_APP_NAME"
    else
...
# ── arguments ───────────────────────────────────────────────────────────────
# Nothing at all is the default jsonq subject. --selftest runs the
# assertions-are-load-bearing proof and exits; the --bin family swaps the
# subject for an already-built binary (see the header).
SELFTEST=0
while [ "$#" -gt 0 ]; do
    case "$1" in
        --selftest)
            ...
            ;;
        --bin)
            [ "${2:-}" != "" ] || die "--bin needs a path"
            SUBJECT_BIN="$2"
            shift 2
            ;;
        --app-name)
            [ "${2:-}" != "" ] || die "--app-name needs a value"
            SUBJECT_APP_NAME="$2"
            shift 2
            ;;
        --probe-args)
            [ "${2:-}" != "" ] || die "--probe-args needs a value"
            PROBE_ARGS="$2"
            shift 2
            ;;
        --probe-expect)
            [ "${2:-}" != "" ] || die "--probe-expect needs a value"
            PROBE_EXPECT="$2"
            shift 2
            ;;
        *)
            die "unknown argument '$1' (expected --selftest, --bin, --app-name, --probe-args, --probe-expect, or nothing)"
            ;;
    esac
done

if [ -n "$SUBJECT_BIN" ]; then
    [ -f "$SUBJECT_BIN" ] || die "subject binary not found: $SUBJECT_BIN"
    [ -x "$SUBJECT_BIN" ] || die "subject binary is not executable: $SUBJECT_BIN"
    if [ "$SUBJECT_APP_NAME" = "jsonq" ]; then
        SUBJECT_APP_NAME="$(basename "$SUBJECT_BIN")"
    fi
    case "$SUBJECT_APP_NAME" in
        ''|.*|*[!A-Za-z0-9._-]*)
            die "app name must match [A-Za-z0-9][A-Za-z0-9._-]* — got '$SUBJECT_APP_NAME'" ;;
    esac
else
    # The default subject is compiled here, so its toolchain is this gate's
    # own prerequisite — not the --bin subject's, which is already built.
    need_tool cc
    [ -f "tools/jsonq.c" ] ||
        die "subject source tools/jsonq.c not found — the subject moved, update this gate"
fi

if [ "$SELFTEST" -eq 1 ]; then
    if [ -n "$SUBJECT_BIN" ]; then
        die "--selftest proves this gate's own assertions and always runs on the default jsonq subject; drop --bin"
    fi
    selftest
    exit 0
fi
...
if prove_two_runs; then
    if [ -n "$SUBJECT_BIN" ]; then
        echo "$GATE_NAME: PASS — two independent bundle runs over $SUBJECT_BIN are byte-identical" \
             "($LAST_MANIFEST_LINES files, signature included, manifest sha256 $LAST_MANIFEST_SHA)" \
             "and the bundled $SUBJECT_APP_NAME launches (exit 0, ${PROBE_EXPECT:+output contained '$PROBE_EXPECT', }output non-empty)"
    else
