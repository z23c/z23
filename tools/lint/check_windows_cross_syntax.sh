...
# Match the real native test profiles. The linked test corpus intentionally
# retains POSIX libc spellings and receives both ZCL_TESTING and the force-
# included compatibility header. Standalone acceptance programs receive test
# declarations but retain their stricter no-compat profile. Production TUs
# receive neither test-only setting.
case "$src" in
    lib/test/src/*_acceptance.c) flags+=(-DZCL_TESTING) ;;
    lib/test/src/*) flags+=(-DZCL_TESTING -include test/windows_compat.h) ;;
