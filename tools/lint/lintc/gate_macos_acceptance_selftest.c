...
static int g_st_rc = 0;

static void mac_expect_reject(const char *label, const char *needle,
...
static void mac_expect_accept(const char *label, const char *fixture_path)
{
    struct mac_result r;
    mac_validate_pub(fixture_path, k_catalog, &r);
    if (r.rc != 0) {
...
static void mac_expect_make_reject(const char *label, const char *fixture_path)
{
...
static void mac_expect_package_reject(const char *label,
...
static int mac_st_group_state(const char *sandbox, const char *matrix)
{
    char p[6][512];
    int np = 0;
#define PLANT(name, content) \
    ...
    mac_expect_accept("0: a copy of the real matrix passes (positive control)",
                      p[0]);
    ...
    PLANT("state.def", t);
    mac_expect_reject("A: an illegal capability state is caught",
                      "invalid state", p[1]);
    ...
    if (mac_replace1_buf(matrix, "test_tor)",
        ...
        return die("z23-lint: derived buffer overflow\n", "");
    PLANT("group.def", t);
    mac_expect_reject("B: a capability naming an unregistered test group is caught",
    ...
    mac_expect_reject("B2: an unregistered group in a non-last position is caught",
    ...
    if (mac_replace1_buf(matrix,
        ...
        return die("z23-lint: derived buffer overflow\n", "");
    PLANT("reason.def", t);
    mac_expect_reject("C: a capability with no typed reason is caught",
                      "no typed reason", p[4]);
    ...
    if (mac_replace1_buf(matrix, "ZCL_MACOS_CAPABILITY(tor,",
        ...
        return die("z23-lint: derived buffer overflow\n", "");
    PLANT("drift.def", t);
    mac_expect_reject("D: drift in the closed capability set is caught",
...
static void mac_st_group_missing(const char *sandbox)
{
    char p[512];
    snprintf(p, sizeof p, "%s/empty.def", sandbox);
    mac_plant(sandbox, "empty.def", "/* no rows */\n");
    mac_expect_reject("E: an empty capability matrix fails closed",
                      "no rows", p);

    snprintf(p, sizeof p, "%s/not_here.def", sandbox);
    mac_expect_reject("F: a missing capability matrix fails closed",
                      "missing capability matrix", p);
}

/* Cases G-J: required-baseline and exact-set drift. */
static int mac_st_group_required(const char *sandbox, const char *matrix)
{
    char p[4][512];
    int np = 0;
#define PLANT(name, content) \
    ...
    mac_expect_reject("G: deletion from the required baseline is caught",
    ...
    mac_expect_reject("H: an unknown required group is caught",
    ...
    mac_expect_reject("I: a same-size registered mutation violates the exact set",
                      "required test set drift", p[2]);

    if (mac_replace1_buf(matrix, "test_tor)", "test_json)", t, sizeof t))
        return die("z23-lint: derived buffer overflow\n", "");
    PLANT("capability_exact_set.def", t);
    mac_expect_reject("J: same-size registered capability evidence drift is caught",
...
static int mac_st_group_contracts(const char *sandbox, const char *matrix)
{
    char p[4][512];
    int np = 0;
#define PLANT(name, content) \
    ...
    mac_expect_reject("K: a legal Seatbelt package downgrade is caught",
    ...
    mac_expect_reject("L: Seatbelt cannot promote resident confinement",
    ...
    mac_expect_reject("M: package evidence reassignment is caught",
    ...
    mac_expect_reject("N: resident evidence reassignment is caught",
...
static int mac_st_group_wiring(const char *sandbox, const char *makefile,
                               const char *accept)
{
    static char t[SEL_BUF];
    ...
    mac_expect_make_reject("O: deleting the native recipe is caught",
                          no_recipe_path);
    ...
    mac_expect_make_reject("P: deleting release-member prerequisites is caught",
                          no_z23_path);

    if (mac_drop_lines_containing_buf(accept, "build_release.sh\" --bin", t,
                                      sizeof t))
        return die("z23-lint: derived buffer overflow\n", "");
    mac_expect_package_reject("Q: deleting the canonical runtime cut is caught",
                              t);

    if (mac_drop_lines_containing_buf(accept, "runtime/z23\" code guide", t,
                                      sizeof t))
        return die("z23-lint: derived buffer overflow\n", "");
    mac_expect_package_reject("R: deleting packaged-node execution is caught",
                              t);
    return 0;
}
...
int check_macos_acceptance_selftest(void)
{
    ...
    int rc = mac_st_group_state(sandbox, matrix);
    if (rc == 0) {
        mac_st_group_missing(sandbox);
        rc = mac_st_group_required(sandbox, matrix);
    }
    if (rc == 0)
        rc = mac_st_group_contracts(sandbox, matrix);
    if (rc == 0)
        rc = mac_st_group_wiring(sandbox, makefile, accept);
    if (rc) {
        (void)rap_rm_rf(sandbox);
        return rc;
    }
