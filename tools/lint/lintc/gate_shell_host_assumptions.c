...
enum {
    SHL_PATH = 768, SHL_FILES_MAX = 2048, SHL_ROWS_MAX = 4096,
    SHL_LOGICAL_MAX = 16384, SHL_VIOL_MAX = 512, SHL_VIOL_LINE = 640
};

static const char k_gate[] = "check-shell-host-assumptions";
...
static int shl_table_find(const struct shl_table *t, const char *kind, const char *path)
{
    for (int i = 0; i < t->n; i++)
...
static int shl_load_baseline(const char *path, struct shl_table *allowed, int *present)
{
    allowed->n = 0;
    *present = 0;
    FILE *f = fopen(path, "r");
