...
static int zpkg_quoted_pair_ok(const char *p, size_t len,
                               const char **q1, const char **q1e,
                               const char **q2, const char **q2e)
{
    const char *lim = p + len;
    *q1 = memchr(p, '"', len);
    *q1e = *q1 ? memchr(*q1 + 1, '"', (size_t)(lim - *q1 - 1)) : NULL;
    *q2 = *q1e ? memchr(*q1e + 1, '"', (size_t)(lim - *q1e - 1)) : NULL;
    *q2e = *q2 ? memchr(*q2 + 1, '"', (size_t)(lim - *q2 - 1)) : NULL;
    return *q1e && *q2e && *q1e > *q1 + 1 && *q2e > *q2 + 1;
}
static int zpkg_parse_rows(const char *text, struct zpkg_row *rows, int cap, int *n)
{
    ...
    while (*p) {
        ...
        if (len >= 14 && memcmp(p, "ZCODE_PACKAGE(", 14) == 0) {
            const char *q1, *q1e, *q2, *q2e;
            if (zpkg_quoted_pair_ok(p, len, &q1, &q1e, &q2, &q2e)) {
...
static int zpr_run_rederive(char *cap, size_t capn)
{
    int code = 0;
    if (capture_cmd("tools/scripts/zcode_registry_rederive.sh --selftest", cap, capn, &code)
        || zpr_replay(cap))
        return 2;
    if (code != 0) return 1;
    char mk[256];
    ...
    if (access(k_zpr_checker, X_OK) != 0)
        return fprintf(stderr, ZPRF "missing %s\n", k_zpr_checker) < 0
                   ? die("z23-lint: write failed\n", "") : 1;
    if (capture_cmd("tools/scripts/zcode_registry_rederive.sh --check", cap, capn, &code)
        || zpr_replay(cap))
        return 2;
    if (code != 0) return 1;
    return 0;
}
static int zpr_run_pkg_sources(char pkg_src[][RS_PATH], int *ns)
{
    ...
    *ns = zpr_sort_unique(pkg_src, acc.n);
    return 0;
}
static int zpr_run_monolith_sources(char *cap, size_t capn, char mono[][RS_PATH], int *nm)
{
    int code = 0;
    if (capture_cmd("make -s --no-print-directory print-zcode-monolith-lib-sources", cap, capn, &code))
        return 2;
    if (code != 0) return 1;
    regex_t re;
    ...
    if (err) return err;
    *nm = 0;
    int rc = zpr_take_match(cap, &re, mono, ZMONO_MAX, nm);
    regfree(&re); if (rc) return rc;
    return 0;
}
static int zpr_run_reject_empty(int ns, int nm)
{
    if (ns == 0 || nm == 0)
...
static int zpr_run_exact_once(char pkg_src[][RS_PATH], int ns, char mono[][RS_PATH], int nm)
{
    char msg[1024];
    for (int i = 0; i < ns; i++) {
...
static int zpr_run_host_optional(char mono[][RS_PATH], int nm)
{
    char msg[1024];
    char host[64] = "";
    struct utsname u;
...
static int zpr_run_plat_groups(char mono[][RS_PATH], int nm)
{
    char msg[1024];
    for (size_t i = 0; i < sizeof k_zpr_groups / sizeof k_zpr_groups[0]; i++) {
        char chosen[RS_PATH];
...
static int zpr_run_codec_cursor(char *cap)
{
    char msg[1024];
    int code = 0;
...
static int zpr_run_codec_priv(char *cap, size_t capn)
{
    char msg[1024];
    int code = 0;
...
int check_zcode_package_registry_run(int argc, char **argv)
{
    ...
    int rc;
    if ((rc = zpr_run_rederive(cap, sizeof cap))) return rc;
    static char pkg_src[ZSRC_MAX][RS_PATH], mono[ZMONO_MAX][RS_PATH];
    int ns = 0, nm = 0;
    if ((rc = zpr_run_pkg_sources(pkg_src, &ns))) return rc;
    if ((rc = zpr_run_monolith_sources(cap, sizeof cap, mono, &nm))) return rc;
    if ((rc = zpr_run_reject_empty(ns, nm))) return rc;
    if ((rc = zpr_run_exact_once(pkg_src, ns, mono, nm))) return rc;
    if ((rc = zpr_run_host_optional(mono, nm))) return rc;
    if ((rc = zpr_run_plat_groups(mono, nm))) return rc;
    if ((rc = zpr_run_codec_cursor(cap))) return rc;
    if ((rc = zpr_run_codec_priv(cap, sizeof cap))) return rc;
    return zpr_run_success(ns);
}

static int zpr_st_exact_msgs(char *msg, size_t cap, int *bad)
{
    *bad |= zpr_fmt_exact(msg, cap, "a.c", 0) || strcmp(msg, ZPRF "a.c appears 0 times in LIB_SRCS");
    *bad |= zpr_fmt_exact(msg, cap, "a.c", 2) || strcmp(msg, ZPRF "a.c appears 2 times in LIB_SRCS");
    return 0;
}
static int zpr_st_group_hits(char *msg, size_t mcap, char *chosen, size_t ccap,
                             char mono[][RS_PATH], int *bad)
{
    if (ovf(snprintf(mono[0], RS_PATH, "%s",
        ...
        return 2;
    int pc = zpr_group_hits(k_zpr_groups[0], mono, 1, chosen, ccap);
    *bad |= pc != 1 || strcmp(chosen, "platform/modules/platform/src/os_sandbox_linux.c");
    pc = zpr_group_hits(k_zpr_groups[0], mono, 0, chosen, ccap);
    *bad |= pc != 0 || zpr_fmt_plat(msg, mcap, 0, k_zpr_groups[0])
        || !strstr(msg, "appears 0 times in LIB_SRCS");
    pc = zpr_group_hits(k_zpr_groups[0], mono, 2, chosen, ccap);
    *bad |= pc != 2 || zpr_fmt_plat(msg, mcap, 2, k_zpr_groups[0])
        || !strstr(msg, "appears 2 times in LIB_SRCS") || !strstr(msg, k_zpr_groups[0]);
    return 0;
}
static int zpr_st_host(char *msg, size_t mcap, char *ok, size_t ocap, int *bad)
{
    *bad |= zpr_host_expected("Linux") != 1 || zpr_host_expected("Linuxfoo") != 1
        || zpr_host_expected("Darwin") != 0;
    *bad |= zpr_fmt_host_ok(ok, ocap, k_zpr_optional[0], 1)
    ...
    *bad |= zpr_fmt_host(msg, mcap, k_zpr_optional[0], 0, 1, "Linux")
        || !strstr(msg, "expected 1 on Linux");
    return 0;
}
static int zpr_st_codec(char *msg, size_t mcap, int *bad)
{
    *bad |= zpr_fmt_codec_miss(msg, mcap, k_zpr_codec[0])
        || strcmp(msg, ZPRF "contexts/commons/modules/vcs/src/package_release.c does not use the bounded codec cursor");
    *bad |= zpr_fmt_codec_priv(msg, mcap)
        || strcmp(msg, ZPRF "package release/recipe/lock restored a private codec");
    return 0;
}
static int zpr_st_excluded_hits(char mono[][RS_PATH], int *bad)
{
...
int check_zcode_package_registry_selftest(void)
{
    ...
    int bad = 0;
    if (zpr_st_exact_msgs(msg, sizeof msg, &bad)) return 2;
    if (zpr_st_group_hits(msg, sizeof msg, chosen, sizeof chosen, mono, &bad)) return 2;
    if (zpr_st_host(msg, sizeof msg, ok, sizeof ok, &bad)) return 2;
    if (zpr_st_codec(msg, sizeof msg, &bad)) return 2;
    if (zpr_st_excluded_hits(mono, &bad)) return 2;
...
static int zps_take_dep_name(const char *line, const struct zpkg_row *rows, int nr,
                            ...
                            char out[][RS_PATH], int cap, int *n)
{
    *n = 0; int in_deps = 0;
    for (const char *p = text; *p; ) {
        ...
        if (in_deps) {
            int rc = zps_take_dep_name(line, rows, nr, out, cap, n);
            if (rc) return rc;
        }
        p = eol ? eol + 1 : p + len;
    }
...
static int zps_take_c_file(const char *dir, const char *sub, const char *nm,
                           int subset, const char *man,
                           char out[][RS_PATH], int cap, int *n)
{
    ...
    if (ovf(snprintf(rel, sizeof rel, "%s/%s", sub, nm), sizeof rel)
        ...
        return 2;
    if (!(subset && !strstr(man, needle))) {
        if (*n >= cap) return ovf(*n + 1, (size_t)cap);
        if (ovf(snprintf(out[*n], RS_PATH, "%s", full), RS_PATH)) return 2;
        (*n)++;
    }
    return 0;
}
                        ...
                        char out[][RS_PATH], int cap, int *n)
{
    ...
    for (int i = 0; i < nd; i++) {
        ...
        if (rc == 0 && nm[0] != '.' && nl > 2 && nm[nl - 2] == '.' && nm[nl - 1] == 'c')
            rc = zps_take_c_file(dir, sub, nm, subset, man, out, cap, n);
        free(names[i]);
    }
    free(names);
    return rc;
...
static int zps_collect_dep_incs(const char *name, char deps[][RS_PATH], int nd,
                                char incs[][RS_PATH], int *ni)
{
    int missing = 0;
    for (int i = 0; i < nd; i++) {
...
static int zps_compile_each_src(const char *name, const char *cc, const char *flags,
                                char incs[][RS_PATH], int ni,
                                char srcs[][RS_PATH], int ns,
                                const char *obj, const char *errp, int *compiled)
{
    int fail = 0;
    for (int i = 0; i < ns; i++) {
        int code = 0;
        (*compiled)++;
        int rc = zps_cc(cc, flags, incs, ni, srcs[i], obj, errp, &code);
        if (rc) return rc;
        if (code != 0) {
                               ...
                               const char *work, int *compiled)
{
    ...
    int rc = zpkg_slurp_path(man, text, sizeof text), ni = 1, nd = 0;
    ...
    if (rc) return rc;
    rc = zps_collect_dep_incs(name, deps, nd, incs, &ni);
    if (rc) return rc;
    ...
    if ((rc = zps_pkg_sources(dir, text, srcs, ZSRC_MAX, &ns))
        ...
        return rc ? rc : 2;
    return zps_compile_each_src(name, cc, flags, incs, ni, srcs, ns, obj, errp, compiled);
}

static int zps_run_prepare(char *flags, size_t fcap, struct zpkg_row *rows, int *nr,
                           const char **cc, char *tmpl, size_t tcap, char **work)
{
    char root[4096];
    if (cic_repo_root(root, sizeof root) || chdir(root) != 0) {
        fputs("FAIL: cannot reach repo root\n", stderr); return 2;
    }
    int rc = zps_quick_flags(flags, fcap);
    if (rc) return rc;
    if (!flags[0])
    ...
    *cc = env_or("CC", "cc");
    if (ovf(snprintf(tmpl, tcap, "%s/z23-lint-zps-XXXXXX",
                     env_or("TMPDIR", "/tmp")), tcap))
        return 2;
    *work = mkdtemp(tmpl);
    if (!*work) return die("z23-lint: mkdir failed: %s\n", tmpl);
    return 0;
}
static int zps_run_compile_all(const struct zpkg_row *rows, int nr, const char *cc,
                               const char *flags, const char *work,
                               int *compiled, int *violations)
{
    for (int i = 0; i < nr; i++) {
        int rc = zps_compile_package(rows[i].name, zpkg_dir_of(rows, nr, rows[i].name),
...
static int zps_run_report(int nr, int compiled, int violations)
{
    if (compiled == 0)
...
int check_zcode_package_standalone_run(int argc, char **argv)
{
    ...
    int nr = 0, rc;
    const char *cc = NULL;
    char tmpl[4096], *work = NULL;
    if ((rc = zps_run_prepare(flags, sizeof flags, rows, &nr, &cc, tmpl, sizeof tmpl, &work)))
        return rc;
    int compiled = 0, violations = 0;
    if ((rc = zps_run_compile_all(rows, nr, cc, flags, work, &compiled, &violations)))
        return rc;
    if ((rc = rap_rm_rf(work))) return rc;
    return zps_run_report(nr, compiled, violations);
}
...
static int zps_st_parse_rows(struct zpkg_row *rows, int *nr, int *bad)
{
    const char *reg =
...
static int zps_st_filter_and_deps(const struct zpkg_row *rows, int nr, int *bad)
{
    ...
    char out[8][RS_PATH], deps[8][RS_PATH];
    int n = 0, nd = 0;
...
static int zps_st_message_shapes(int *bad)
{
    char msg[256];
...
static int zps_st_write_fixtures(const char *work, char *okp, char *badp,
                                 char *obj, char *errp, size_t cap)
{
    if (ovf(snprintf(okp, cap, "%s/ok.c", work), cap)
        || ovf(snprintf(badp, cap, "%s/bad.c", work), cap)
        || ovf(snprintf(obj, cap, "%s/obj.o", work), cap)
        || ovf(snprintf(errp, cap, "%s/err.txt", work), cap)
        || csr_write(okp, "int z23_lint_zps_ok;\n") || csr_write(badp, "int {\n"))
        { rap_rm_rf(work); return 2; }
    return 0;
}
static int zps_st_compile_preview(int *bad)
{
    ...
    incs[0][0] = '\0';
    if (zps_st_write_fixtures(work, okp, badp, obj, errp, sizeof okp))
        return 2;
    int code = 0;
...
int check_zcode_package_standalone_selftest(void)
{
    ...
    int nr = 0, bad = 0;
    if (zps_st_parse_rows(rows, &nr, &bad)) return 2;
    if (zps_st_filter_and_deps(rows, nr, &bad)) return 2;
    if (zps_st_message_shapes(&bad)) return 2;
    if (zps_st_compile_preview(&bad)) return 2;
