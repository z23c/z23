...
validate() {
    ...
    [ "$union_count" = 43 ] ||
        die "exact evidence union drift: expected 43 groups; observed $union_count"
...
case "${1:---check}" in
    --check)
        validate
        printf 'macos-acceptance: capability matrix + required baseline PASS (43 exact groups)\n'
        ;;
    --groups)
        validate
        exact_groups
