...
validate() {
    ...
    [ "$union_count" = 38 ] ||
        die "exact evidence union drift: expected 38 groups; observed $union_count"
...
case "${1:---check}" in
    --check)
        validate
        printf 'macos-acceptance: capability matrix + required baseline PASS (38 exact groups)\n'
        ;;
    --groups)
        validate
        exact_groups
