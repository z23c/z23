...
validate() {
    ...
    [ "$union_count" = 40 ] ||
        die "exact evidence union drift: expected 40 groups; observed $union_count"
...
case "${1:---check}" in
    --check)
        validate
        printf 'macos-acceptance: capability matrix + required baseline PASS (40 exact groups)\n'
        ;;
    --groups)
        validate
        exact_groups
