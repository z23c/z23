 ...
 * Purpose: proves file_copy() produces a private, byte-identical copy and
 * refuses a directory destination. On Windows it additionally proves
 * dir_copy() refuses without creating the tree and that block-file cleanup
 * spares blk*.dat. Works under the scratch datadir given as argv[1]. */
