 ...
 * Purpose: proves platform_private_file_link_no_clobber() publishes argv[1]
 * to argv[2] only under the caller VERIFIED file identity -- a deliberately
 * corrupted identity is refused with the destination left absent, and the
 * correct identity is idempotent across repeats. */
