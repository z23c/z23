 ...
 * Purpose: Windows acceptance for stale-lock recovery --
 * os_proc_pid_liveness() separates a running pid from a dead one, and a
 * verified private-file handle denies name substitution (MoveFileEx) so
 * only an identity-matched retire can release the lock. */
